    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          if (pba->has_nufld == _TRUE_){
            for (n_nufld = 0; n_nufld < pba->N_nufld; n_nufld++){
              class_store_double(dataptr,tk[ppt->index_tp_delta_nufld1+n_nufld],ppt->has_source_delta_nufld,storeidx);
            }
          }
        ...
        if (ppt->has_velocity_transfers == _TRUE_) {
          ...
          if (pba->has_nufld == _TRUE_){
            for (n_nufld = 0; n_nufld < pba->N_nufld; n_nufld++){
              class_store_double(dataptr,tk[ppt->index_tp_theta_nufld1+n_nufld],ppt->has_source_theta_nufld,storeidx);
            }
          }
        ...
        class_store_double_or_default(dataptr,-tk[ppt->index_tp_delta_nufld1]/k2,ppt->has_source_delta_nufld,storeidx,0.0);
...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    class_store_columntitle(titles,"-T_nufld/k2",_TRUE_);
...
int perturbations_init(
  ...
  if (pba->has_nufld == _TRUE_) {
    ...
    class_test ((ppr->nufld_fluid_approximation < nufldfa_mb) ||
                (ppr->nufld_fluid_approximation > nufldfa_none),
                ppt->error_message,
                "your nufld_fluid_approximation is set to %d, out of range defined in perturbations.h",ppr->nufld_fluid_approximation);

    if (ppt->has_nc_density == _TRUE_) { // nufld_MOD: I don't know if this is important.
      if (ppt->perturbations_verbose > 0) {
        fprintf(stdout," -> [WARNING:] You request the number count Cl's in presence of non-cold dark matter.\n    Like in all previous CLASS and CLASSgal versions, this will be inferred from the total matter density,\n    but it could make much more sense physically to compute it from the CDM+baryon density only.\n    To get the latter behavior you would just need to change one line in transfer.c:\n    search there for a comment starting with 'use here delta_cb'\n");
      }
    }
  ...
  if (ppt->has_tensors == _TRUE_) {
    ...
    ppt->evolve_tensor_nufld = _FALSE_;
    ...
    case (tm_exact):
      ...
      if (pba->has_ncdm == _TRUE_)
        ...
        if (pba->has_nufld == _TRUE_)
        ppt->evolve_tensor_nufld = _TRUE_;
      break;
    }
  }
...
int perturbations_indices(
  ...
  ppt->has_source_delta_nufld = _FALSE_;
  ppt->has_source_delta_tot = _FALSE_;
  ...
  ppt->has_source_theta_nufld = _FALSE_;
  ppt->has_source_theta_tot = _FALSE_;
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_nufld == _TRUE_)
          ppt->has_source_delta_nufld = _TRUE_;
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if (pba->has_nufld == _TRUE_)
          ppt->has_source_theta_nufld = _TRUE_;
      ...
      class_define_index(ppt->index_tp_delta_nufld1,ppt->has_source_delta_nufld,index_type,pba->N_nufld);
      ...
      class_define_index(ppt->index_tp_theta_nufld1,ppt->has_source_theta_nufld,index_type,pba->N_nufld);
...
int perturbations_workspace_init(
  ...
  if (_scalars_) {
    ...
    if (pba->has_nufld == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_nufld);
  ...
  if (_tensors_) {
    ...
    if (pba->has_ur == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_ur);
    if (pba->has_ncdm == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_ncdm);
    if (pba->has_nufld == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_nufld); // nufld_MOD: Should I change this?
  }
  ...
  if (_scalars_) {
    ...
    class_define_index(ppw->index_ap_nufldfa,pba->has_nufld,index_ap,1);
  ...
  if (_scalars_) {
    ...
    if (pba->has_nufld == _TRUE_) {
      ppw->approx[ppw->index_ap_nufldfa]=(int)nufldfa_off;
    }
  }
  ...
  if (_scalars_) {
    ...
    if ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
      ...
      // nufld_MOD: This is a bit weird, I don't understand if this should be in here.
      class_alloc(ppw->delta_nufld,pba->N_nufld*sizeof(double),ppt->error_message);
      class_alloc(ppw->theta_nufld,pba->N_nufld*sizeof(double),ppt->error_message);
      class_alloc(ppw->shear_nufld,pba->N_nufld*sizeof(double),ppt->error_message);
...
int perturbations_solve(
  ...
  int n_ncdm,n_nufld,is_early_enough;
  ...
  if (pba->has_nufld == _TRUE_) {
    for (n_nufld=0; n_nufld < pba->N_nufld; n_nufld++) {
      class_test(fabs(ppw->pvecback[pba->index_bg_p_nufld1+n_nufld]/ppw->pvecback[pba->index_bg_rho_nufld1+n_nufld]-1./3.)>ppr->tol_nufld_initial_w,
                 ppt->error_message,
                 "your choice of initial time for integrating wavenumbers is inappropriate: it corresponds to a time at which the nufld species number %d is not ultra-relativistic anymore, with w=%g, p=%g and rho=%g\n",
                 n_nufld,
                 ppw->pvecback[pba->index_bg_p_nufld1+n_nufld]/ppw->pvecback[pba->index_bg_rho_nufld1+n_nufld],
                 ppw->pvecback[pba->index_bg_p_nufld1+n_nufld],
                 ppw->pvecback[pba->index_bg_rho_nufld1+n_nufld]);
    }
  }
...
int perturbations_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      if ((pba->has_nufld == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
        for (n_nufld=0; n_nufld < pba->N_nufld; n_nufld++){
          sprintf(tmp,"delta_nufld[%d]",n_nufld);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          sprintf(tmp,"theta_nufld[%d]",n_nufld);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
    ...
    for (index_switch=1; index_switch<interval_number; index_switch++) {
      ...
      if (ppt->perturbations_verbose>2) {
        ...
        if (_scalars_) {
          ...
          if (pba->has_nufld == _TRUE_) {
            if ((interval_approx[index_switch-1][ppw->index_ap_nufldfa]==(int)nufldfa_off) &&
                (interval_approx[index_switch][ppw->index_ap_nufldfa]==(int)nufldfa_on)) {
              fprintf(stdout,"Mode k=%e: will switch on nufld fluid approximation at tau=%e\n",k,interval_limit[index_switch]);
            }
          }
        }
...
int perturbations_vector_init(
  ...
  int index_pt;
  int l;
  int n_ncdm,n_nufld,index_q,ncdm_l_size,nufld_l_size; //nufld_MOD: Do we need another index_q specific for nufld?
  double rho_plus_p_ncdm,rho_plus_p_nufld,q,q2,epsilon,a,factor;
  ...
  ppv->l_max_nufld = NULL;
  ppv->q_size_nufld = NULL;
  ...
  if (_scalars_) {
    ...
    if (pba->has_nufld == _TRUE_) {
      ...
      class_alloc(ppv->q_size_nufld,ppv->N_nufld*sizeof(double),ppt->error_message);

      for (n_nufld = 0; n_nufld < pba->N_nufld; n_nufld++){
        // Set value of ppv->l_max_nufld:
        if (ppw->approx[ppw->index_ap_nufldfa] == (int)nufldfa_off){
          /* reject inconsistent values of the number of mutipoles in ultra relativistic neutrino hierarchy */
          class_test(ppr->l_max_nufld < 4,
                     ppt->error_message,
                     "ppr->l_max_nufld=%d should be at least 4, i.e. we must integrate at least over first four momenta of non-cold dark matter perturbed phase-space distribution",n_nufld);
          //Copy value from precision parameter:
          ppv->l_max_nufld[n_nufld] = ppr->l_max_nufld;
        ...
        index_pt += (ppv->l_max_nufld[n_nufld]+1)*ppv->q_size_nufld[n_nufld];
      }
    }
  ...
  if (pa_old == NULL) {
    ...
    if (_scalars_) {
      ...
      if (pba->has_nufld == _TRUE_) {
        ...
        class_test(ppw->approx[ppw->index_ap_nufldfa] == (int)nufldfa_on,
                   ppt->error_message,
                   "scalar initial conditions assume nufld fluid approximation turned off");
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
            ...
            for (index_q=0; index_q < ppv->q_size_nufld[n_nufld]; index_q++){
              for (l=0; l<=ppv->l_max_nufld[n_nufld];l++){
                ...
                ppv->y[ppv->index_pt_psi0_nufld1+index_pt] =
                  ppw->pv->y[ppw->pv->index_pt_psi0_nufld1+index_pt];
                index_pt++;
              }
            }
          }
     ...
     /* -- case of switching on nufld fluid
         approximation. Provide correct initial conditions to new set
         of variables */
...
int perturbations_vector_free(
  ...
  if (pv->l_max_nufld != NULL) free(pv->l_max_nufld);
  if (pv->q_size_nufld != NULL) free(pv->q_size_nufld);  
  free(pv->y);
  free(pv->dy);
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  double delta_dr=0;
  double q,epsilon,k2;
  int index_q,n_ncdm,n_nufld,idx;
  ...
  if (_scalars_) {
    ...
    if (pba->has_nufld == _TRUE_) {
      for (n_nufld=0; n_nufld<pba->N_nufld; n_nufld++){
        rho_r += ppw->pvecback[pba->index_bg_rho_nufld1 + n_nufld];
        rho_nu += ppw->pvecback[pba->index_bg_rho_nufld1 + n_nufld];
      }
    }
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      /* all relativistic relics: ur, early ncdm, dr, nufld */

      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_nufld == _TRUE_) || (pba->has_dr == _TRUE_) || (pba->has_idr == _TRUE_)) {
    ...
    if ((ppt->has_nid == _TRUE_) && (index_ic == ppt->index_ic_nid)) {
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_ncdm == _FALSE_) && (pba->has_nufld == _FALSE_),
                 ppt->error_message,
                 "not consistent to ask for NID in absence of ur, ncdm or nufld species!");
    ...
    if ((ppt->has_niv == _TRUE_) && (index_ic == ppt->index_ic_niv)) {
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_ncdm == _FALSE_) && (pba->has_ncdm == _FALSE_),
                 ppt->error_message,
                 "not consistent to ask for NIV in absence of ur, ncdm or nufld species!");
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      /** - (e) In any gauge, we should now implement the relativistic initial conditions in ur, nufld and ncdm variables */
    ...
    if (pba->has_nufld == _TRUE_) {
      ...
      for (n_nufld=0; n_nufld < pba->N_nufld; n_nufld++){
        ...
        for (index_q=0; index_q < ppw->pv->q_size_nufld[n_nufld]; index_q++) {
          ...
          epsilon = sqrt(q*q+a*a*pba->M_nufld[n_nufld]*pba->M_nufld[n_nufld]);

          ppw->pv->y[idx] = -0.25 * delta_ur * pba->dlnf0_dlnq_nufld[n_nufld][index_q];

          ppw->pv->y[idx+1] =  -epsilon/3./q/k*theta_ur* pba->dlnf0_dlnq_nufld[n_nufld][index_q];

          ppw->pv->y[idx+2] = -0.5 * shear_ur * pba->dlnf0_dlnq_nufld[n_nufld][index_q];

          ppw->pv->y[idx+3] = -0.25 * l3_ur * pba->dlnf0_dlnq_nufld[n_nufld][index_q];
...
int perturbations_approximations(
  ...
  if (_scalars_) {
    ...
    if (pba->has_nufld == _TRUE_) {
      ...
      if ((tau/tau_k > ppr->nufld_fluid_trigger_tau_over_tau_k) &&
          (ppr->nufld_fluid_approximation != nufldfa_none)) {

        ppw->approx[ppw->index_ap_nufldfa] = (int)nufldfa_on;
      }
      else {
        ppw->approx[ppw->index_ap_nufldfa] = (int)nufldfa_off;
      }
...
int perturbations_timescale(
  ...
  if ((ppt->has_scalars == _TRUE_) && (pppaw->index_md == ppt->index_md_scalars)) {
    ...
    *timescale = tau_h;
    ...
    if ((ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) || (pba->has_ncdm == _TRUE_) || (pba->has_nufld == _TRUE_)) // nufld_MOD: Did I do this right?
...
int perturbations_total_stress_energy(
  ...
  double w_fld,dw_over_da_fld,integral_fld;
  double gwncdm;
  double rho_delta_nufld=0.;
  double rho_plus_p_theta_nufld=0.;
  double rho_plus_p_shear_nufld=0.;
  ...
  if (_scalars_) {
    ...
    /* infer delta_cb abd theta_cb (perturbations from CDM and baryons) before adding ncdm or nufld */
    ...
    if (pba->has_nufld == _TRUE_) {
      ...
      if (ppt->has_source_delta_m == _TRUE_) {
        for (n_nufld=0; n_nufld < pba->N_nufld; n_nufld++){
          ...
          rho_m += ppw->pvecback[pba->index_bg_rho_nufld1+n_nufld];
        }
      }
      if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
        for (n_nufld=0; n_nufld < pba->N_nufld; n_nufld++){
          rho_plus_p_theta_m += (ppw->pvecback[pba->index_bg_rho_nufld1+n_nufld]+ppw->pvecback[pba->index_bg_p_nufld1+n_nufld])
            *ppw->theta_nufld[n_nufld]; // contribution to [(rho+p)theta]_matter
          rho_plus_p_m += (ppw->pvecback[pba->index_bg_rho_nufld1+n_nufld]+ppw->pvecback[pba->index_bg_p_nufld1+n_nufld]);
        }
      }
    }
      ...
      if (ppt->tensor_method == tm_massless_approximation) {
        ...
        if (pba->has_nufld == _TRUE_) {
          for (n_nufld = 0; n_nufld < pba->N_nufld; n_nufld++) {
            ...
            rho_relativistic += 3.*ppw->pvecback[pba->index_bg_p_nufld1+n_nufld];
          }
        }
      }
    ...
    if (ppt->evolve_tensor_nufld == _TRUE_){
      ...
      for (n_nufld=0; n_nufld < pba->N_nufld; n_nufld++){
        ...
        for (index_q=0; index_q < ppw->pv->q_size_nufld[n_nufld]; index_q ++) {
          ...
          gwnufld += q2*q2/epsilon*pba->w_nufld[n_nufld][index_q]*(1./15.*y[idx]+2./21.*y[idx+2]+1./35.*y[idx+4]);

          //Jump to next momentum bin:
          idx+=(ppw->pv->l_max_nufld[n_nufld]+1);
        }

        gwnufld *= -_SQRT6_*4*a2*factor;

        ppw->gw_source += gwnufld;
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    /* delta_nufld1 */
    if (ppt->has_source_delta_nufld == _TRUE_) {
      for (index_tp = ppt->index_tp_delta_nufld1; index_tp < ppt->index_tp_delta_nufld1+pba->N_nufld; index_tp++) {
        _set_source_(index_tp) = ppw->delta_nufld[index_tp - ppt->index_tp_delta_nufld1]
          + 3.*a_prime_over_a*(1+pvecback[index_tp - ppt->index_tp_delta_nufld1 + pba->index_bg_p_nufld1]
                               /pvecback[index_tp - ppt->index_tp_delta_nufld1 + pba->index_bg_rho_nufld1])*theta_over_k2; // N-body gauge correction
      }
    }
    ...
    /* theta_nufld1 */
    if (ppt->has_source_theta_nufld == _TRUE_) {
      for (index_tp = ppt->index_tp_theta_nufld1; index_tp < ppt->index_tp_theta_nufld1+pba->N_nufld; index_tp++) {
        _set_source_(index_tp) = ppw->theta_nufld[index_tp - ppt->index_tp_theta_nufld1]
          + theta_shift; // N-body gauge correction
      }
    }
  }
...
int perturbations_print_variables(double tau,
  ...
  if (pba->has_nufld == _TRUE_){
    class_alloc(delta_nufld, sizeof(double)*pba->N_nufld,error_message);
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_nufld == _TRUE_) {
        for (n_nufld=0; n_nufld < pba->N_nufld; n_nufld++){
          /** - --> TODO: gauge transformation of delta, deltaP/rho (?) and theta using -= 3aH(1+w_nufld) alpha for delta. */
        }
      }
    ...
    if ((pba->has_nufld == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
      for (n_nufld=0; n_nufld < pba->N_nufld; n_nufld++){
        class_store_double(dataptr, delta_nufld[n_nufld], _TRUE_, storeidx);
...
int variables_nufld_from_tower(struct perturbations_workspace * ppw,
  ...
  for (int n_nufld = 0; n_nufld < pba->N_nufld; n_nufld++) {
    ...
    if (ceff2     != NULL) {
      ...
      if (k <= pba->k_cut_nufld[n_nufld]) { 
        ceff2[n_nufld]  = (k2*delta_p_nufld/rho_nufld+3*a*H*ca2_nufld_mass*(1+w_nufld_mass)*rho_plus_p_theta_nufld/(rho_nufld+p_nufld))/
                          (k2*rho_delta_nufld/rho_nufld+3*a*H*(1+w_nufld_mass)*rho_plus_p_theta_nufld/(rho_nufld+p_nufld));
      } else {
...
int perturbations_derivs(double tau,
  ...
  double w_nufld, w_prime_nufld;
  double ceff2_nufld_bltz[3]; // NUFLD_TODO: This index is not very general, but whatever. Should allocate and bla bla bla.
  double * ceff2_nufld_bltz_ptr = ceff2_nufld_bltz;
  double delta_rho_nufld_bltz[3];
  double * delta_rho_nufld_bltz_ptr = delta_rho_nufld_bltz;
  double delta_p_nufld_bltz[3];
  ...
  if (_scalars_) {
    ...
    /** - ---> non-cold dark matter with generalized Boltzmann hierarchy (nufld): massive neutrinos, WDM, etc. */
    ...
    if (pba->has_nufld == _TRUE_) {
      ...
      for (n_nufld=0; n_nufld<pv->N_nufld; n_nufld++) {
        ...
        // Sound speed in the fluid rest frame, computed from the Boltzmann tower
        cf2_nufld = ceff2_nufld_bltz[n_nufld];
