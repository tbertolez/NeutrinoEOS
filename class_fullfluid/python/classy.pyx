...
cdef class Class:
    ...
    property Omega_bltz: # Did I do this correctly BLTZ_MOD?
        def __get__(self):
            return self.ba.Omega0_bltz_tot
    ...
    def get_pk_all(self, k, z, nonlinear = True, cdmbar = False, z_axis_in_k_arr = 0):
        ...
        ispkcb = cdmbar and not (self.ba.Omega0_ncdm_tot == 0.) and not (self.ba.Omega0_bltz_tot == 0.) # BLTZ_MOD: Is this correct? 
   ...
   #################################
    ...
    def Om_bltz(self, z):
        ...
        Return the bltz density fraction (exactly, the ratio of quantities defined by Class as
        Sum_m [ index_bg_rho_bltz1 + n ], with n=0...N_bltz-1, and index_bg_rho_crit in the background module)
        ...
        if self.ba.has_bltz == True:
            ...
            rho_bltz = 0.
            for n in range(self.ba.N_bltz):
                rho_bltz += pvecback[self.ba.index_bg_rho_bltz1+n]
            Om_bltz = rho_bltz/pvecback[self.ba.index_bg_rho_crit]
    ...
    def get_current_derived_parameters(self, names):
        ...
        for name in names:
            ...
            elif name == 'm_bltz_in_eV':
                self.compute(["background"])
                value = self.ba.m_bltz_in_eV[0]
            elif name == 'm_bltz_tot':
                value = self.ba.Omega0_bltz_tot*self.ba.h*self.ba.h*93.14
            elif name == 'Neff':
                value = self.ba.Neff
            elif name == 'Omega_m':
...
make        nonlinear_scale_cb(z, z_size)
        ...
        #cdef double *k_nl
        #k_nl = <double*> calloc(z_size,sizeof(double))
        if (self.ba.Omega0_ncdm_tot == 0.) and (self.ba.Omega0_bltz_tot == 0.): # BLTZ_MOD: Did I do this correctly?
            raise CosmoSevereError(
...
#                raise CosmoSevereError(self.fo.error_message)
    ...
    def get_sources(self):
        ...
        if self.pt.has_source_delta_bltz:
            for ibltz in range(self.ba.N_bltz):
              indices.append(self.pt.index_tp_delta_bltz1+ibltz)
              names.append("delta_bltz[{}]".format(ibltz))
        ...
        if self.pt.has_source_theta_bltz:
            for ibltz in range(self.ba.N_bltz):
              indices.append(self.pt.index_tp_theta_bltz1+ibltz)
              names.append("theta_bltz[{}]".format(ibltz))
