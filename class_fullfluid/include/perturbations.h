...
enum bltzfa_flags {bltzfa_off, bltzfa_on};
...
enum bltzfa_method {bltzfa_mb,bltzfa_hu,bltzfa_CLASS,bltzfa_none};
...
struct perturbations
{
  ...
  short evolve_tensor_bltz;             /**< will we evolve bltz tensor perturbations (if we have bltz species and we use the exact method) ? */
  ...
  short has_source_delta_bltz; /**< do we need source for delta of all non-cold dark matter species with general Boltzmann hierarchy (e.g. massive neutrinos)? */
  ...
  short has_source_theta_bltz; /**< do we need source for theta of all non-cold dark matter species with general Boltzmann hierarchy (e.g. massive neutrinos)? */
  ...
  int index_tp_delta_bltz1; /**< index value for delta of first non-cold dark matter species (e.g. massive neutrinos) */
  ...
  int index_tp_theta_bltz1; /**< index value for theta of first non-cold dark matter species (e.g. massive neutrinos) */
...
struct perturbations_vector
{
  ...
  int index_pt_psi0_bltz1; /**< first multipole of perturbation of first bltz species, Psi_0 */
  int N_bltz;		/**< number of distinct non-cold-dark-matter (bltz) species */
  int* l_max_bltz;	/**< mutipole l at which Boltzmann hierarchy is truncated (for each bltz species) */
  int* q_size_bltz;	/**< number of discrete momenta (for each bltz species) */
...
struct perturbations_workspace
{
  ...
  */
  ...
  double * delta_bltz;	/**< relative density perturbation of each bltz species */
  double * theta_bltz;	/**< velocity divergence theta of each bltz species */
  double * shear_bltz;	/**< shear for each bltz species */
  ...
  int index_ap_bltzfa; /**< index for bltz fluid approximation */
