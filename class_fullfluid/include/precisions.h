 ...
 * For models like ncdm or bltz, the code may decide to start the integration earlier.
 */
 ...
 * In the nonrelativistic limit, this could be estimated using M=density/number density
 */
class_precision_parameter(tol_M_bltz,double,1.e-7)
/**
 * Tolerance on the relative precision of the integration over
 * non-cold dark matter phase-space distributions.
 */
class_precision_parameter(tol_bltz,double,1.e-3)
/**
 * Tolerance on the relative precision of the integration over
 * non-cold dark matter phase-space distributions in the synchronous gauge.
 */
class_precision_parameter(tol_bltz_synchronous,double,1.e-3)
/**
 * Tolerance on the relative precision of the integration over
 * non-cold dark matter phase-space distributions in the newtonian gauge.
 */
class_precision_parameter(tol_bltz_newtonian,double,1.e-5)
/**
 * Tolerance on the relative precision of the integration over
 * non-cold dark matter phase-space distributions during the background evolution.
 */
class_precision_parameter(tol_bltz_bg,double,1.e-5)
/**
 * Tolerance on the initial deviation of non-cold dark matter from being fully relativistic.
 * Using w = pressure/density, this quantifies the maximum deviation from 1/3. (for relativistic species)
 */
class_precision_parameter(tol_bltz_initial_w,double,1.e-3)
 ...
 * algorithms BLTZ_MOD: I don't know what this parameter is doing and if bltz should use it to, probably yes.
 */
...
class_precision_parameter(l_max_bltz,int,17)   /**< number of momenta in Boltzmann hierarchy for relativistic neutrino/relics (scalar), at least 4 */
...
class_precision_parameter(bltz_fluid_approximation,int,bltzfa_CLASS) /**< method for non-cold dark matter fluid approximation */
/**
...
/**
 * when to switch off bltz (massive neutrinos with generalized Boltzmann hierarchy / non-cold
 * relics) fluid approximation
 */
class_precision_parameter(bltz_fluid_trigger_tau_over_tau_k,double,31.0)
/**
