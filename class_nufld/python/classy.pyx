...
cdef class Class:
    ...
    property Omega_nufld: # Did I do this correctly nufld_MOD?
        def __get__(self):
            return self.ba.Omega0_nufld_tot
    ...
    def get_pk_all(self, k, z, nonlinear = True, cdmbar = False, z_axis_in_k_arr = 0):
        ...
        ispkcb = cdmbar and not (self.ba.Omega0_ncdm_tot == 0.) and not (self.ba.Omega0_nufld_tot == 0.) # nufld_MOD: Is this correct? 
   ...
   #################################
    ...
    def Om_nufld(self, z):
        ...
        Return the nufld density fraction (exactly, the ratio of quantities defined by Class as
        Sum_m [ index_bg_rho_nufld1 + n ], with n=0...N_nufld-1, and index_bg_rho_crit in the background module)
        ...
        if self.ba.has_nufld == True:
            ...
            rho_nufld = 0.
            for n in range(self.ba.N_nufld):
                rho_nufld += pvecback[self.ba.index_bg_rho_nufld1+n]
            Om_nufld = rho_nufld/pvecback[self.ba.index_bg_rho_crit]
    ...
    def get_current_derived_parameters(self, names):
        ...
        for name in names:
            ...
            elif name == 'm_nufld_in_eV':
                self.compute(["background"])
                value = self.ba.m_nufld_in_eV[0]
            elif name == 'm_nufld_tot':
                value = self.ba.Omega0_nufld_tot*self.ba.h*self.ba.h*93.14
            elif name == 'Neff':
                value = self.ba.Neff
            elif name == 'Omega_m':
...
make        nonlinear_scale_cb(z, z_size)
        ...
        #cdef double *k_nl
        #k_nl = <double*> calloc(z_size,sizeof(double))
        if (self.ba.Omega0_ncdm_tot == 0.) and (self.ba.Omega0_nufld_tot == 0.): # nufld_MOD: Did I do this correctly?
            raise CosmoSevereError(
...
#                raise CosmoSevereError(self.fo.error_message)
    ...
    def get_sources(self):
        ...
        if self.pt.has_source_delta_nufld:
            for inufld in range(self.ba.N_nufld):
              indices.append(self.pt.index_tp_delta_nufld1+inufld)
              names.append("delta_nufld[{}]".format(inufld))
        ...
        if self.pt.has_source_theta_nufld:
            for inufld in range(self.ba.N_nufld):
              indices.append(self.pt.index_tp_theta_nufld1+inufld)
              names.append("theta_nufld[{}]".format(inufld))
