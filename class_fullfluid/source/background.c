 ...
 * * routines related to bltz written by T. Bertólez-Martínez in 2023
...
int background_functions(
  ...
  int n_ncdm;
  /* background bltz quantities */
  double rho_bltz,p_bltz,pseudo_p_bltz;
  /* index for n_bltz species */
  int n_bltz;
  ...
  if (pba->has_bltz == _TRUE_) {
    ...
    for (n_bltz=0; n_bltz<pba->N_bltz; n_bltz++) {
      ...
      /* function returning background bltz[n_bltz] quantities (only
         those for which non-NULL pointers are passed) */
      class_call(background_bltz_momenta(
                                         pba->q_bltz_bg[n_bltz],
                                         pba->w_bltz_bg[n_bltz],
                                         pba->q_size_bltz_bg[n_bltz],
                                         pba->M_bltz[n_bltz],
                                         pba->factor_bltz[n_bltz],
                                         1./a-1.,
                                         NULL,
                                         &rho_bltz,
                                         &p_bltz,
      ...
      pvecback[pba->index_bg_rho_bltz1+n_bltz] = rho_bltz;
      rho_tot += rho_bltz;
      pvecback[pba->index_bg_p_bltz1+n_bltz] = p_bltz;
      p_tot += p_bltz;
      pvecback[pba->index_bg_pseudo_p_bltz1+n_bltz] = pseudo_p_bltz;
      /** See e.g. Eq. A6 in 1811.00904. */
      dp_dloga += (pseudo_p_bltz - 5*p_bltz);

      /* (3 p_bltz1) is the "relativistic" contribution to rho_bltz1 */
      rho_r += 3.* p_bltz;

      /* (rho_bltz1 - 3 p_bltz1) is the "non-relativistic" contribution
         to rho_bltz1 */
      rho_m += rho_bltz - 3.* p_bltz;
    }
  }
...
int background_free_input(
  ...
  if (pba->Omega0_bltz_tot != 0.) {
    for (k=0; k<pba->N_bltz; k++) {
      free(pba->q_bltz[k]);
      free(pba->w_bltz[k]);
      free(pba->q_bltz_bg[k]);
      free(pba->w_bltz_bg[k]);
      free(pba->dlnf0_dlnq_bltz[k]);
    }
    free(pba->bltz_quadrature_strategy);
    free(pba->bltz_input_q_size);
    free(pba->bltz_qmax);
    free(pba->q_bltz);
    free(pba->w_bltz);
    free(pba->q_bltz_bg);
    free(pba->w_bltz_bg);
    free(pba->dlnf0_dlnq_bltz);
    free(pba->q_size_bltz);
    free(pba->q_size_bltz_bg);
    free(pba->M_bltz);
    free(pba->T_bltz);
    free(pba->ksi_bltz);
    free(pba->deg_bltz);
    free(pba->Omega0_bltz);
    free(pba->m_bltz_in_eV);
    free(pba->factor_bltz);
    if (pba->got_files!=NULL)
      free(pba->got_files);
    if (pba->bltz_psd_files!=NULL)
      free(pba->bltz_psd_files);
    if (pba->bltz_psd_parameters!=NULL)
      free(pba->bltz_psd_parameters);
  }
...
int background_indices(
  ...
  /* - indices for bltz. We only define the indices for bltz1
     (density, pressure, pseudo-pressure), the other bltz indices
     are contiguous */
  class_define_index(pba->index_bg_rho_bltz1,pba->has_bltz,index_bg,pba->N_bltz);
  class_define_index(pba->index_bg_p_bltz1,pba->has_bltz,index_bg,pba->N_bltz);
  class_define_index(pba->index_bg_pseudo_p_bltz1,pba->has_bltz,index_bg,pba->N_bltz);
...
/**
 * This is the routine where the distribution function f0(q) of each
 * bltz species is specified (it is the only place to modify if you
 * need a partlar f0(q))
 * BLTZ_MOD this is important: Where do we get the distribution function from?
 *
 * @param pbadist Input:  structure containing all parameters defining f0(q)
 * @param q       Input:  momentum
 * @param f0      Output: phase-space distribution
...
int background_bltz_distribution(
                                 void * pbadist,
                                 double q,
                                 double * f0
                                 ) {
  ...
  pbadist_local = pbadist;          /* restore actual format of pbadist */
  pba = pbadist_local->pba;         /* extract the background structure from it */
  param = pba->bltz_psd_parameters; /* extract the optional parameter list from it */
  n_bltz = pbadist_local->n_bltz;   /* extract index of bltz species under consideration */
  ksi = pba->ksi_bltz[n_bltz];      /* extract chemical potential */

  /** - shall we interpolate in file, or shall we use analytical formula below? */

  /** - a) deal first with the case of interpolating in files */
  if (pba->got_files[n_bltz]==_TRUE_) {
  ...
  /** - b) deal now with case of reading analytical function */
    ...
    /**
       Next enter your analytic expression(s) for the p.s.d.'s. If
       you need different p.s.d.'s for different species, put each
       p.s.d inside a condition, like for instance: if (n_bltz==2)
       {*f0=...}.  Remember that n_bltz = 0 refers to the first
       species.
    */
    ...
    *f0 = 1.0/pow(2*_PI_,3)*(1./(exp(q-ksi)+1.) +1./(exp(q+ksi)+1.));
    ...
    if (_FALSE_) {
      ...
      /* We must use the list of extra parameters read in input, stored in the
         bltz_psd_parameter list, extracted above from the structure
         and now called param[..] */

      /* check that this list has been read */
      class_test(param == NULL,
                 pba->error_message,
                 "Analytic expression wants to use 'bltz_psd_parameters', but they have not been entered!");
      ...
      *f0=0.0;
      for (i=0;i<3;i++) {
        ...
        *f0 += mixing_matrix[i][n_bltz]*1.0/pow(2*_PI_,3)*(1./(exp(q-pba->ksi_bltz[i])+1.) +1./(exp(q+pba->ksi_bltz[i])+1.));
...
int background_bltz_test_function(
                                  void * pbadist,
                                  double q,
                                  double * test
                                  ) {
...
/**
 * This function finds optimal quadrature weights for each bltz
 * species
 *
 * @param ppr Input: precision structure
 * @param pba Input/Output: background structure
...
int background_bltz_init(
  ...
  /* Allocate pointer arrays: */
  class_alloc(pba->q_bltz, sizeof(double*)*pba->N_bltz,pba->error_message);
  class_alloc(pba->w_bltz, sizeof(double*)*pba->N_bltz,pba->error_message);
  class_alloc(pba->q_bltz_bg, sizeof(double*)*pba->N_bltz,pba->error_message);
  class_alloc(pba->w_bltz_bg, sizeof(double*)*pba->N_bltz,pba->error_message);
  class_alloc(pba->dlnf0_dlnq_bltz, sizeof(double*)*pba->N_bltz,pba->error_message);

  /* Allocate pointers: */
  class_alloc(pba->q_size_bltz,sizeof(int)*pba->N_bltz,pba->error_message);
  class_alloc(pba->q_size_bltz_bg,sizeof(int)*pba->N_bltz,pba->error_message);
  class_alloc(pba->factor_bltz,sizeof(double)*pba->N_bltz,pba->error_message);

  for (k=0, filenum=0; k<pba->N_bltz; k++) {
    pbadist.n_bltz = k;
    pbadist.q = NULL;
    pbadist.tablesize = 0;
    /*Do we need to read in a file to interpolate the distribution function? */
    if ((pba->got_files!=NULL)&&(pba->got_files[k]==_TRUE_)) {
      psdfile = fopen(pba->bltz_psd_files+filenum*_ARGUMENT_LENGTH_MAX_,"r");
      class_test(psdfile == NULL,pba->error_message,
                 "Could not open file %s!",pba->bltz_psd_files+filenum*_ARGUMENT_LENGTH_MAX_);
      // Find size of table:
      for (row=0,status=2; status==2; row++) {
        status = fscanf(psdfile,"%lf %lf",&tmp1,&tmp2);
      }
    ...
    /* Handle perturbation qsampling: */
    if (pba->bltz_quadrature_strategy[k]==qm_auto) {
      /** Automatic q-sampling for this species */
      class_alloc(pba->q_bltz[k],_QUADRATURE_MAX_*sizeof(double),pba->error_message);
      class_alloc(pba->w_bltz[k],_QUADRATURE_MAX_*sizeof(double),pba->error_message);

      class_call(get_qsampling(pba->q_bltz[k],
                               pba->w_bltz[k],
                               &(pba->q_size_bltz[k]),
                               _QUADRATURE_MAX_,
                               ppr->tol_bltz,
                               pbadist.q,
                               pbadist.tablesize,
                               background_bltz_test_function,
                               background_bltz_distribution,
                               &pbadist,
                               pba->error_message),
                 pba->error_message,
                 pba->error_message);
      pba->q_bltz[k]=realloc(pba->q_bltz[k],pba->q_size_bltz[k]*sizeof(double));
      pba->w_bltz[k]=realloc(pba->w_bltz[k],pba->q_size_bltz[k]*sizeof(double));


      if (pba->background_verbose > 0) {
        printf("bltz species i=%d sampled with %d points for purpose of perturbation integration\n",
               k+1,
               pba->q_size_bltz[k]);
      }

      /* Handle background q_sampling: */
      class_alloc(pba->q_bltz_bg[k],_QUADRATURE_MAX_BG_*sizeof(double),pba->error_message);
      class_alloc(pba->w_bltz_bg[k],_QUADRATURE_MAX_BG_*sizeof(double),pba->error_message);

      class_call(get_qsampling(pba->q_bltz_bg[k],
                               pba->w_bltz_bg[k],
                               &(pba->q_size_bltz_bg[k]),
                               _QUADRATURE_MAX_BG_,
                               ppr->tol_bltz_bg,
                               pbadist.q,
                               pbadist.tablesize,
      ...
      pba->q_bltz_bg[k]=realloc(pba->q_bltz_bg[k],pba->q_size_bltz_bg[k]*sizeof(double));
      pba->w_bltz_bg[k]=realloc(pba->w_bltz_bg[k],pba->q_size_bltz_bg[k]*sizeof(double));

      /** - in verbose mode, inform user of number of sampled momenta
          for background quantities */
      if (pba->background_verbose > 0) {
        printf("bltz species i=%d sampled with %d points for purpose of background integration\n",
               k+1,
               pba->q_size_bltz_bg[k]);
      }
    }
      ...
      /** Manual q-sampling for this species. Same sampling used for both perturbation and background sampling, since this will usually be a high precision setting anyway */
      pba->q_size_bltz_bg[k] = pba->bltz_input_q_size[k];
      pba->q_size_bltz[k] = pba->bltz_input_q_size[k];
      class_alloc(pba->q_bltz_bg[k],pba->q_size_bltz_bg[k]*sizeof(double),pba->error_message);
      class_alloc(pba->w_bltz_bg[k],pba->q_size_bltz_bg[k]*sizeof(double),pba->error_message);
      class_alloc(pba->q_bltz[k],pba->q_size_bltz[k]*sizeof(double),pba->error_message);
      class_alloc(pba->w_bltz[k],pba->q_size_bltz[k]*sizeof(double),pba->error_message);
      class_call(get_qsampling_manual(pba->q_bltz[k],
                                      pba->w_bltz[k],
                                      pba->q_size_bltz[k],
                                      pba->bltz_qmax[k],
                                      pba->bltz_quadrature_strategy[k],
                                      pbadist.q,
                                      pbadist.tablesize,
                                      background_bltz_distribution,
                                      &pbadist,
                 ...
                 pba->error_message,
                 pba->error_message);
      for (index_q=0; index_q<pba->q_size_bltz[k]; index_q++) {
        pba->q_bltz_bg[k][index_q] = pba->q_bltz[k][index_q];
        pba->w_bltz_bg[k][index_q] = pba->w_bltz[k][index_q];
      }
      /** - in verbose mode, inform user of number of sampled momenta
          for background quantities */
      if (pba->background_verbose > 0) {
        printf("bltz species i=%d sampled with %d points for purpose of background andperturbation integration using the manual method\n",
               k+1,
               pba->q_size_bltz[k]);
      }
    }

    class_alloc(pba->dlnf0_dlnq_bltz[k],
                pba->q_size_bltz[k]*sizeof(double),
                pba->error_message);


    for (index_q=0; index_q<pba->q_size_bltz[k]; index_q++) {
      q = pba->q_bltz[k][index_q];
      class_call(background_bltz_distribution(&pbadist,q,&f0),
                 pba->error_message,pba->error_message);
      ...
      for (tolexp=_PSD_DERIVATIVE_EXP_MIN_; tolexp<_PSD_DERIVATIVE_EXP_MAX_; tolexp++) {
        ...
        if (index_q == 0) {
          dq = MIN((0.5-ppr->smallest_allowed_variation)*q,2*exp(tolexp)*(pba->q_bltz[k][index_q+1]-q));
        }
        else if (index_q == pba->q_size_bltz[k]-1) {
          dq = exp(tolexp)*2.0*(pba->q_bltz[k][index_q]-pba->q_bltz[k][index_q-1]);
        }
        else{
          dq = exp(tolexp)*(pba->q_bltz[k][index_q+1]-pba->q_bltz[k][index_q-1]);
        }

        class_call(background_bltz_distribution(&pbadist,q-2*dq,&f0m2),
                   pba->error_message,pba->error_message);
        class_call(background_bltz_distribution(&pbadist,q+2*dq,&f0p2),
                   pba->error_message,pba->error_message);

        if (fabs((f0p2-f0m2)/f0)>sqrt(ppr->smallest_allowed_variation)) break;
      }

      class_call(background_bltz_distribution(&pbadist,q-dq,&f0m1),
                 pba->error_message,pba->error_message);
      class_call(background_bltz_distribution(&pbadist,q+dq,&f0p1),
                 pba->error_message,pba->error_message);
      //5 point estimate of the derivative:
      df0dq = (+f0m2-8*f0m1+8*f0p1-f0p2)/12.0/dq;
      //printf("df0dq[%g] = %g. dlf=%g ?= %g. f0 =%g.\n",q,df0dq,q/f0*df0dq,
      //Avoid underflow in extreme tail:
      if (fabs(f0)==0.)
        pba->dlnf0_dlnq_bltz[k][index_q] = -q; /* valid for whatever f0 with exponential tail in exp(-q) */
      else
        pba->dlnf0_dlnq_bltz[k][index_q] = q/f0*df0dq;
    }

    pba->factor_bltz[k]=pba->deg_bltz[k]*4*_PI_*pow(pba->T_cmb*pba->T_bltz[k]*_k_B_,4)*8*_PI_*_G_
      /3./pow(_h_P_/2./_PI_,3)/pow(_c_,7)*_Mpc_over_m_*_Mpc_over_m_;
...
/**
 * For a given bltz species: given the quadrature weights, the mass
 * and the redshift, find background quantities by a quick weighted
 * sum over.  Input parameters passed as NULL pointers are not
 * evaluated for speed-up
 *
...
int background_bltz_momenta(
                            ...
                            double * p,   // pressure
                            double * drho_dM,  // d rho / d M used in next function
                            double * pseudo_p  // pseudo-p used in bltz fluid approx
                            ) {
...
/**
 * When the user passed the density fraction Omega_bltz or
 * omega_bltz in input but not the mass, infer the mass with Newton iteration method.
 *
 * @param ppr    Input: precision structure
 * @param pba    Input/Output: background structure
 * @param n_bltz Input: index of bltz species
 */

int background_bltz_M_from_Omega(
                                 struct precision *ppr,
                                 struct background *pba,
                                 int n_bltz
                                 ) {
  ...
  rho0 = pba->H0*pba->H0*pba->Omega0_bltz[n_bltz]; /*Remember that rho is defined such that H^2=sum(rho_i) */
  M = 0.0;

  background_bltz_momenta(pba->q_bltz_bg[n_bltz],
                          pba->w_bltz_bg[n_bltz],
                          pba->q_size_bltz_bg[n_bltz],
                          M,
                          pba->factor_bltz[n_bltz],
  ...
  class_test(rho0<rho,pba->error_message,
             "The value of Omega for the %dth species, %g, is less than for a massless species! It should be atleast %g. Check your input.",
             n_bltz,pba->Omega0_bltz[n_bltz],pba->Omega0_bltz[n_bltz]*rho/rho0);
  ...
  for (iter=1; iter<=maxiter; iter++) {
    ...
    if ((M+deltaM)<0.0) deltaM = -M/2.0; /* Avoid overshooting to negative M value. */
    M += deltaM; /* Update value of M.. */
    if (fabs(deltaM/M)<ppr->tol_M_bltz) {
      /* Accuracy reached.. */
      pba->M_bltz[n_bltz] = M;
      break;
    }
...
int background_checks(
  ...
  if (pba->background_verbose > 0) {
    ...
    if (pba->has_bltz == _TRUE_) {
      ...
      /* loop over bltz species */
      for (n_bltz=0;n_bltz<pba->N_bltz; n_bltz++) {

        /* inform if p-s-d read in files */
        if (pba->got_files[n_bltz] == _TRUE_) {
          printf(" -> bltz species i=%d read from file %s\n",n_bltz+1,pba->bltz_psd_files+filenum*_ARGUMENT_LENGTH_MAX_);
          filenum++;
        }

        /* inform the user also about the value of the bltz
           masses in eV and about */
        printf(" -> non-cold dark matter species with i=%d has m_i = %e eV (so m_i / omega_i =%e eV)\n",
               n_bltz+1,
               pba->m_bltz_in_eV[n_bltz],
               pba->m_bltz_in_eV[n_bltz]*pba->deg_bltz[n_bltz]/pba->Omega0_bltz[n_bltz]/pba->h/pba->h);

        /* call this function to get rho_bltz */
        background_bltz_momenta(pba->q_bltz_bg[n_bltz],
                                pba->w_bltz_bg[n_bltz],
                                pba->q_size_bltz_bg[n_bltz],
                                0.,
        ...
        /* inform user of the contribution of each species to
           radiation density (in relativistic limit): should be
           between 1.01 and 1.02 for each active neutrino species;
           evaluated as rho_bltz/rho_nu_rel where rho_nu_rel is the
           density of one neutrino in the instantaneous decoupling
           limit, i.e. assuming T_nu=(4/11)^1/3 T_gamma (this comes
           from the definition of N_eff) */
        rho_nu_rel = 56.0/45.0*pow(_PI_,6)*pow(4.0/11.0,4.0/3.0)*_G_/pow(_h_P_,3)/pow(_c_,7)*
        ...
        printf(" -> bltz species i=%d sampled with %d (resp. %d) points for purpose of background (resp. perturbation) integration. In the relativistic limit it gives Delta N_eff = %g\n",
               n_bltz+1,
               pba->q_size_bltz_bg[n_bltz],
               pba->q_size_bltz[n_bltz],
               rho_bltz_rel/rho_nu_rel);
      }
    }
...
int background_solve(
  ...
  for (n_bltz=0;n_bltz<pba->N_bltz; n_bltz++) {
    /* here we add to non-free-streaming matter as: any non-relatistic species with a dimensionless ratio m/T bigger than a threshold ppr->M_nfsm_threshold; if this threshold is of the order of 10^4, this corresponds to the condition "becoming non-relativistic during radiation domination". Beware: this definition won't work in the case in which the user passes a customised p.s.d. for bltz, such that M_bltz is not defined.  */
    if (pba->M_bltz[n_bltz] > ppr->M_nfsm_threshold) {
      pba->Omega0_nfsm += pba->Omega0_bltz[n_bltz];
    }
  }
...
int background_initial_conditions(
  ...
  double rho_bltz, p_bltz, rho_bltz_rel_tot=0.;
  double f,Omega_rad, rho_rad;
  int counter,is_early_enough,n_ncdm,n_bltz;
  double scf_lambda;
  ...
  */
  ...
  if (pba->has_bltz == _TRUE_) {
    ...
    for (counter=0; counter < _MAX_IT_; counter++) {
      ...
      for (n_bltz=0; n_bltz<pba->N_bltz; n_bltz++) {
        ...
        class_call(background_bltz_momenta(pba->q_bltz_bg[n_bltz],
                                           pba->w_bltz_bg[n_bltz],
                                           pba->q_size_bltz_bg[n_bltz],
                                           pba->M_bltz[n_bltz],
                                           pba->factor_bltz[n_bltz],
                   ...
                   pba->error_message,
                   pba->error_message);
        rho_bltz_rel_tot += 3.*p_bltz;
        if (fabs(p_bltz/rho_bltz-1./3.)>ppr->tol_bltz_initial_w) {
          is_early_enough = _FALSE_;
        }
      }
      if (is_early_enough == _TRUE_) {
    ...
    class_test(counter == _MAX_IT_,
               pba->error_message,
               "Search for initial scale factor a such that all bltz species are relativistic failed.");
  }
  ...
  if (pba->has_bltz == _TRUE_) {
    /** - We must add the relativistic contribution from bltz species */
    rho_rad += rho_bltz_rel_tot;
  }
  if (pba->has_dcdm == _TRUE_) {
...
int background_output_titles(
  ...
  if (pba->has_bltz == _TRUE_) {
    for (n=0; n<pba->N_bltz; n++) {
      sprintf(tmp,"(.)rho_bltz[%d]",n);
      class_store_columntitle(titles,tmp,_TRUE_);
      sprintf(tmp,"(.)p_bltz[%d]",n);
      class_store_columntitle(titles,tmp,_TRUE_);
    }
  }
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    if (pba->has_bltz == _TRUE_) {
      for (n=0; n<pba->N_bltz; n++) {
        class_store_double(dataptr,pvecback[pba->index_bg_rho_bltz1+n],_TRUE_,storeidx);
        class_store_double(dataptr,pvecback[pba->index_bg_p_bltz1+n],_TRUE_,storeidx);
      }
    }
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if (pba->N_bltz > 0) {
      for (index_bltz=0;index_bltz<pba->N_bltz;++index_bltz) {
        printf("-> %-26s%-4d Omega = %-15g , omega = %-15g\n","Non-Cold Species Nr.",index_bltz+1,pba->Omega0_bltz[index_bltz],pba->Omega0_bltz[index_bltz]*pba->h*pba->h);
        budget_neutrino+=pba->Omega0_bltz[index_bltz];
        budget_matter+=pba->Omega0_bltz[index_bltz];
      }
    }
    ...
    if (pba->N_bltz > 0) {
      printf(" - Non-Free-Streaming Matter      Omega = %-15g , omega = %-15g \n",pba->Omega0_nfsm,pba->Omega0_nfsm*pba->h*pba->h);
      printf(" - Boltzmann Non-Cold DM          Omega = %-15g , omega = %-15g \n",budget_neutrino,budget_neutrino*pba->h*pba->h);
    }
