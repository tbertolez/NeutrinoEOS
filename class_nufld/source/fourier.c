 ...
 * @param k_nl_cb Ouput: k_nl value of the cdm+baryon part only, if there is ncdm (nufld_MOD or nufld? I have doubts about this file)
 * @return the error status
 */
  ...
  if (pfo->method > nl_none) {
    ...
    if (pba->has_nufld == _TRUE_) {
      for (index_nufld=0;index_nufld < pba->N_nufld; index_nufld++){
        if (pba->m_nufld_in_eV[index_nufld] >  _M_EV_TOO_BIG_FOR_HALOFIT_)
          fprintf(stdout,"Warning: Halofit and HMcode are proved to work for CDM, and also with a small HDM component. But it sounds like you are running with a WDM component of mass %f eV, which makes the use of Halofit suspicious.\n",pba->m_nufld_in_eV[index_nufld]);
      }
    }
...
int fourier_indices(
  ...
  pfo->has_pk_m = _TRUE_;
  if ((pba->has_ncdm == _TRUE_)||(pba->has_nufld == _TRUE_)) { // nufld_MOD: Did I do this right?
...
int fourier_halofit(
  ...
  */
  ...
  if ((pfo->has_pk_m == _TRUE_) && (index_pk == pfo->index_pk_m)) {
    fnu = (pba->Omega0_ncdm_tot+pba->Omega0_nufld_tot)/pba->Omega0_m; // nufld_MOD: This makes sense but I don't know.
  }
  else if ((pfo->has_pk_cb == _TRUE_) && (index_pk == pfo->index_pk_cb)) {
    fnu = 0.;
  }
...
int fourier_hmcode(
  ...
  fnu      = (pba->Omega0_ncdm_tot+pba->Omega0_nufld_tot)/Omega0_m; // nufld_MOD: Did I do this right?
  ...
  if (index_pk==pfo->index_pk_cb){
    Omega0_m = Omega0_m - pba->Omega0_ncdm_tot - pba->Omega0_nufld_tot; // nufld_MOD: Did I do this right?
  }
