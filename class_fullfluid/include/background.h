...
struct background
{
  ...
  int N_bltz;                            /**< Number of distinguishable bltz species */
  /* the following parameters help to define tabulated bltz p-s-d passed in file */
  char * bltz_psd_files;                 /**< list of filenames for tabulated p-s-d */
  // int * got_files;                       /**< BLTZ_MOD: What about this? list of flags for each species, set to true if p-s-d is passed through file */
  /* the following parameters help to define the analytical bltz phase space distributions (p-s-d) */
  double * bltz_psd_parameters;          /**< list of parameters for specifying/modifying bltz p.s.d.'s, to be customized for given model
                                            (could be e.g. mixing angles) */
  double * M_bltz;                       /**< vector of masses of non-cold relic: dimensionless ratios m_bltz/T_bltz */
  double * m_bltz_in_eV;                 /**< list of bltz masses in eV (inferred from M_bltz and other parameters above) */
  double * Omega0_bltz, Omega0_bltz_tot; /**< Omega0_bltz for each species and for the total Omega0_bltz */
  double * T_bltz,T_bltz_default;        /**< list of 1st parameters in p-s-d of non-cold relics: relative temperature
                                            T_bltz1/T_gamma; and its default value */
  double * ksi_bltz, ksi_bltz_default;   /**< list of 2nd parameters in p-s-d of non-cold relics: relative chemical potential
                                            ksi_bltz1/T_bltz1; and its default value */
  double * deg_bltz, deg_bltz_default;    /**< vector of degeneracy parameters in factor of p-s-d: 1 for one family of neutrinos
                                             (= one neutrino plus its anti-neutrino, total g*=1+1=2, so deg = 0.5 g*); and its
                                             default value */
  int * bltz_input_q_size; /**< Vector of numbers of q bins */
  double * bltz_qmax;      /**< Vector of maximum value of q */
  ...
  int index_bg_rho_bltz1;     /**< density of first bltz species (others contiguous) */
  int index_bg_p_bltz1;       /**< pressure of first bltz species (others contiguous) */
  int index_bg_pseudo_p_bltz1;/**< another statistical momentum useful in bltza approximation */
  ...
  short has_bltz;      /**< presence of non-cold dark matter with general boltzmann hierarchy? */
  ...
  /**
   *@name - arrays related to sampling and integration of bltz phase space distributions
   */

  //@{

  int * bltz_quadrature_strategy; /**< Vector of integers according to quadrature strategy. */
  double ** q_bltz_bg;  /**< Pointers to vectors of background sampling in q */
  double ** w_bltz_bg;  /**< Pointers to vectors of corresponding quadrature weights w */
  double ** q_bltz;     /**< Pointers to vectors of perturbation sampling in q */
  double ** w_bltz;     /**< Pointers to vectors of corresponding quadrature weights w */
  double ** dlnf0_dlnq_bltz; /**< Pointers to vectors of logarithmic derivatives of p-s-d */
  int * q_size_bltz_bg; /**< Size of the q_bltz_bg arrays */
  int * q_size_bltz;    /**< Size of the q_bltz arrays */
  double * factor_bltz; /**< List of normalization factors for calculating energy density etc.*/
...
// BLTZ_MOD: These lines have not been changed (yet), we can use the same lines as in ncdm

#define _SCALE_BACK_ 0.1  /**< logarithmic step used when searching for an initial scale factor at which ncdm are still relativistic */
