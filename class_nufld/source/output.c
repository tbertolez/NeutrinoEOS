...
int output_tk(
  ...
  if (pop->output_format == camb_format) {
    ...
    class_test(pba->N_nufld>1,
               pop->error_message,
               "you wish to output the transfer functions in CMBFAST/CAMB format but you have more than one non-cold dark matter (nufld) species. The two are not compatible (since CMBFAST/CAMB only have one nufld species): switch to CLASS output format or keep only on nufld species");
