...
int input_read_parameters_species(struct file_content * pfc,
  ...
  if (N_nufld > 0){
    pba->N_nufld = N_nufld;
    if (ppt->gauge == synchronous){
      ppr->tol_nufld = ppr->tol_nufld_synchronous;
    }
    if (ppt->gauge == newtonian){
      ppr->tol_nufld = ppr->tol_nufld_newtonian;
    }

    /** 6.b) Check if filenames for interpolation tables are given */
    /* Read */
    class_read_list_of_integers_or_default("use_nufld_psd_files",pba->got_files,_FALSE_,N_nufld);
    /* Complete set of parameters */
    for (n=0,fileentries=0; n<N_nufld; n++){
      if (pba->got_files[n] == _TRUE_){
        fileentries++;
      }
    }
    if (fileentries > 0) {
      ...
      /* Read */
      class_call(parser_read_list_of_strings(pfc,"nufld_psd_filenames",&entries_read,&(pba->nufld_psd_files),&flag1,errmsg),
                 errmsg,
                 errmsg);
      /* Test */
      class_test(flag1 == _FALSE_,errmsg,
                 "Entry 'use_nufld_files' is found, but no corresponding 'nufld_psd_filenames' were found.");
      class_test(entries_read != fileentries,errmsg,
                 "Number of filenames found (%d) does not match number of _TRUE_ values in use_nufld_files (%d).",
                 entries_read,fileentries);
    }

    /** 6.c) (optional) p.s.d.-parameters */
    /* Read */
    class_call(parser_read_list_of_doubles(pfc,"nufld_psd_parameters",&entries_read,&(pba->nufld_psd_parameters),&flag1,errmsg),
               errmsg,
               errmsg);

    /** 6.d) Mass or Omega of each nufld species */
    /* Read */
    class_read_list_of_doubles_or_default("m_nufld",pba->m_nufld_in_eV,0.0,N_nufld);
    class_read_list_of_doubles_or_default("Omega_nufld",pba->Omega0_nufld,0.0,N_nufld);
    class_read_list_of_doubles_or_default("omega_nufld",pba->M_nufld,0.0,N_nufld);
    for (n=0; n<N_nufld; n++){
      if (pba->M_nufld[n]!=0.0){
        /* Test */
        class_test(pba->Omega0_nufld[n]!=0,errmsg,
                   "You can only enter one of 'Omega_nufld' or 'omega_nufld' for nufld species %d.",n);
        /* Complete set of parameters */
        pba->Omega0_nufld[n] = pba->M_nufld[n]/pba->h/pba->h;
      }
      /* Set default value
         this is the right place for passing the default value of the mass
         (all parameters must have a default value; most of them are defined
         in input_default_params, but the nufld mass is a bit special and
         there is no better place for setting its default value). We put an
         arbitrary value m << 10^-3 eV, i.e. the ultra-relativistic limit. */
      if ((pba->Omega0_nufld[n]==0.0) && (pba->m_nufld_in_eV[n]==0.0)) {
        pba->m_nufld_in_eV[n]=1.e-5;
      }
    }
    ...
    class_call(parser_read_list_of_integers(pfc, "Quadrature strategy", &entries_read, &(pba->nufld_quadrature_strategy), &flag1, errmsg),
               errmsg, errmsg); //Deprecated parameter, still read to keep compatibility
    if (flag1 == _TRUE_) {
      class_test(entries_read != N_nufld, errmsg, "Number of entries in Quadrature strategy, %d, is different from the number of N_cdm species, %d", entries_read, N_nufld);
    }
    else {
    ...
    class_call(parser_read_list_of_doubles(pfc, "Maximum_q", &entries_read, &(pba->nufld_qmax), &flag1, errmsg),
               errmsg, errmsg); //Deprecated parameter, still read to keep compatibility
    if (flag1 == _TRUE_) {
      class_test(entries_read != N_nufld, errmsg, "Number of entries in Maximum_q, %d, is different from the number of N_cdm species, %d", entries_read, N_nufld);
    }
    else {
    ...
    class_call(parser_read_list_of_integers(pfc, "Number of momentum bins", &entries_read, &(pba->nufld_input_q_size), &flag1, errmsg),
               errmsg, errmsg); //Deprecated parameter, still read to keep compatibility
    if (flag1 == _TRUE_) {
      class_test(entries_read != N_nufld, errmsg, "Number of entries in Number of momentum bins, %d, is different from the number of N_cdm species, %d", entries_read, N_nufld);
    }
    else {
      class_read_list_of_integers_or_default("nufld_N_momentum_bins", pba->nufld_input_q_size, 150, N_nufld);
    }

    /** Last step of 6) (i.e. nufld) -- Calculate the masses and momenta */
    class_call(background_nufld_init(ppr,pba),
               pba->error_message,
               errmsg);
    /* Complete set of parameters
       We must calculate M from omega or vice versa if one of them is missing.
    ...
    for (n=0; n < N_nufld; n++){
      if (pba->m_nufld_in_eV[n] != 0.0){
        ...
        if (pba->Omega0_nufld[n] == 0.0){
          ...
          fnu_factor = (pba->H0*pba->H0*pba->Omega0_nufld[n]/rho_nufld);
          pba->factor_nufld[n] *= fnu_factor;
          /* dlnf0dlnq is already computed, but it is independent of any
             normalization of f0. We don't need the factor anymore, but we
             store it nevertheless */
          pba->deg_nufld[n] *=fnu_factor;
        }
      }
      else{
        /* Case of only Omega/omega: */
        class_call(background_nufld_M_from_Omega(ppr,pba,n),
                   pba->error_message,
                   errmsg);
        pba->m_nufld_in_eV[n] = _k_B_/_eV_*pba->T_nufld[n]*pba->M_nufld[n]*pba->T_cmb;
      }
      pba->Omega0_nufld_tot += pba->Omega0_nufld[n];
    }

  }
  class_test(pba->Omega0_nufld_tot<0,errmsg,"You cannot set the nufld density to negative values.");
  if (has_m_budget == _TRUE_) {
    class_test(Omega_m_remaining < pba->Omega0_nufld_tot, errmsg, "Too much energy density from massive species. At this point only %e is left for Omega_m, but requested 'Omega_nufld = %e' (summed over all species)",Omega_m_remaining, pba->Omega0_nufld_tot);
    Omega_m_remaining-= pba->Omega0_nufld_tot;
  }
  ...
  Omega_tot += pba->Omega0_nufld_tot;
  /* Step 1 */
  if (flag1 == _TRUE_){
...
int input_read_parameters_additional(struct file_content* pfc,
  ...
  */
  ...
  if (pba->N_nufld>0) {
    class_test(ppr->nufld_fluid_trigger_tau_over_tau_k==ppr->radiation_streaming_trigger_tau_over_tau_k,
               errmsg,
               "please choose different values for precision parameters nufld_fluid_trigger_tau_over_tau_k and radiation_streaming_trigger_tau_over_tau_k, in order to avoid switching two approximation schemes at the same time");

    class_test(ppr->nufld_fluid_trigger_tau_over_tau_k==ppr->ur_fluid_trigger_tau_over_tau_k,
               errmsg,
               "please choose different values for precision parameters nufld_fluid_trigger_tau_over_tau_k and ur_fluid_trigger_tau_over_tau_k, in order to avoid switching two approximation schemes at the same time");
  }
  ...
  if (pba->Omega0_idr != 0.){
    ...
    class_test(ppr->idr_streaming_trigger_tau_over_tau_k==ppr->nufld_fluid_trigger_tau_over_tau_k,
               errmsg,
               "please choose different values for precision parameters dark_radiation_trigger_tau_over_tau_k and nufld_fluid_trigger_tau_over_tau_k, in order to avoid switching two approximation schemes at the same time");
  }
...
int input_default_params(struct background *pba,
  ...
  pba->nufld_psd_files = NULL;
  /** 6.c) Analytic distribution function */
  pba->nufld_psd_parameters = NULL;
  pba->Omega0_nufld_tot = 0.;
  /** 6.d) --> See read_parameters_background */
  /** 6.e) nufld temperature */
  pba->T_nufld_default = 0.71611; /* this value gives m/omega = 93.14 eV b*/
  pba->T_nufld = NULL;
  /** 6.f) nufld chemical potential */
  pba->ksi_nufld_default = 0.;
  pba->ksi_nufld = NULL;
  /** 6.g) nufld degeneracy parameter */
  pba->deg_nufld_default = 1.;
  pba->deg_nufld = NULL;
  ...
  pba->Omega0_fld = 0.;
  pba->Omega0_scf = 0.;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_nufld_tot-pba->Omega0_dcdmdr - pba->Omega0_idr -pba->Omega0_idm;
  /** 9.a) Omega fluid */
  /** 9.a.1) PPF approximation */
  pba->use_ppf = _TRUE_;
