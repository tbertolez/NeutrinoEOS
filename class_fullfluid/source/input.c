...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /** 6) Non-cold relics with generalized Boltzmann hierarchy (bltz) */
  /** 6.a) Number of non-cold relics */
  /* Read */
  class_read_int("N_bltz",N_bltz);
  /* Complete set of parameters */
  if (N_bltz > 0){
    pba->N_bltz = N_bltz;
    if (ppt->gauge == synchronous){
      ppr->tol_bltz = ppr->tol_bltz_synchronous;
    }
    if (ppt->gauge == newtonian){
      ppr->tol_bltz = ppr->tol_bltz_newtonian;
    }

    /** 6.b) Check if filenames for interpolation tables are given */
    /* Read */
    class_read_list_of_integers_or_default("use_bltz_psd_files",pba->got_files,_FALSE_,N_bltz);
    /* Complete set of parameters */
    for (n=0,fileentries=0; n<N_bltz; n++){
      if (pba->got_files[n] == _TRUE_){
        fileentries++;
      }
    }
    if (fileentries > 0) {
      ...
      /* Read */
      class_call(parser_read_list_of_strings(pfc,"bltz_psd_filenames",&entries_read,&(pba->bltz_psd_files),&flag1,errmsg),
                 errmsg,
                 errmsg);
      /* Test */
      class_test(flag1 == _FALSE_,errmsg,
                 "Entry 'use_bltz_files' is found, but no corresponding 'bltz_psd_filenames' were found.");
      class_test(entries_read != fileentries,errmsg,
                 "Number of filenames found (%d) does not match number of _TRUE_ values in use_bltz_files (%d).",
                 entries_read,fileentries);
    }

    /** 6.c) (optional) p.s.d.-parameters */
    /* Read */
    class_call(parser_read_list_of_doubles(pfc,"bltz_psd_parameters",&entries_read,&(pba->bltz_psd_parameters),&flag1,errmsg),
               errmsg,
               errmsg);

    /** 6.d) Mass or Omega of each bltz species */
    /* Read */
    class_read_list_of_doubles_or_default("m_bltz",pba->m_bltz_in_eV,0.0,N_bltz);
    class_read_list_of_doubles_or_default("Omega_bltz",pba->Omega0_bltz,0.0,N_bltz);
    class_read_list_of_doubles_or_default("omega_bltz",pba->M_bltz,0.0,N_bltz);
    for (n=0; n<N_bltz; n++){
      if (pba->M_bltz[n]!=0.0){
        /* Test */
        class_test(pba->Omega0_bltz[n]!=0,errmsg,
                   "You can only enter one of 'Omega_bltz' or 'omega_bltz' for bltz species %d.",n);
        /* Complete set of parameters */
        pba->Omega0_bltz[n] = pba->M_bltz[n]/pba->h/pba->h;
      }
      /* Set default value
         this is the right place for passing the default value of the mass
         (all parameters must have a default value; most of them are defined
         in input_default_params, but the bltz mass is a bit special and
         there is no better place for setting its default value). We put an
         arbitrary value m << 10^-3 eV, i.e. the ultra-relativistic limit. */
      if ((pba->Omega0_bltz[n]==0.0) && (pba->m_bltz_in_eV[n]==0.0)) {
        pba->m_bltz_in_eV[n]=1.e-5;
      }
    }

    /** 6.e) Temperatures */
    /* Read */
    class_read_list_of_doubles_or_default("T_bltz",pba->T_bltz,pba->T_bltz_default,N_bltz);

    /** 6.f) Chemical potentials */
    /* Read */
    class_read_list_of_doubles_or_default("ksi_bltz",pba->ksi_bltz,pba->ksi_bltz_default,N_bltz);

    /** 6.g) Degeneracy of each bltz species */
    /* Read */
    class_read_list_of_doubles_or_default("deg_bltz",pba->deg_bltz,pba->deg_bltz_default,N_bltz);

    /** 6.h) Quadrature modes, 0 is qm_auto */
    /* Read */
    class_call(parser_read_list_of_integers(pfc, "Quadrature strategy", &entries_read, &(pba->bltz_quadrature_strategy), &flag1, errmsg),
               errmsg, errmsg); //Deprecated parameter, still read to keep compatibility
    if (flag1 == _TRUE_) {
      class_test(entries_read != N_bltz, errmsg, "Number of entries in Quadrature strategy, %d, is different from the number of N_cdm species, %d", entries_read, N_bltz);
    }
    else {
      class_read_list_of_integers_or_default("bltz_quadrature_strategy", pba->bltz_quadrature_strategy, 0, N_bltz);
    }

    /** 6.h.1) qmax, if relevant */
    /* Read */
    class_call(parser_read_list_of_doubles(pfc, "Maximum_q", &entries_read, &(pba->bltz_qmax), &flag1, errmsg),
               errmsg, errmsg); //Deprecated parameter, still read to keep compatibility
    if (flag1 == _TRUE_) {
      class_test(entries_read != N_bltz, errmsg, "Number of entries in Maximum_q, %d, is different from the number of N_cdm species, %d", entries_read, N_bltz);
    }
    else {
      class_read_list_of_doubles_or_default("bltz_maximum_q", pba->bltz_qmax, 15, N_bltz);
    }

    /** 6.h.2) Number of momentum bins */
    class_call(parser_read_list_of_integers(pfc, "Number of momentum bins", &entries_read, &(pba->bltz_input_q_size), &flag1, errmsg),
               errmsg, errmsg); //Deprecated parameter, still read to keep compatibility
    if (flag1 == _TRUE_) {
      class_test(entries_read != N_bltz, errmsg, "Number of entries in Number of momentum bins, %d, is different from the number of N_cdm species, %d", entries_read, N_bltz);
    }
    else {
      class_read_list_of_integers_or_default("bltz_N_momentum_bins", pba->bltz_input_q_size, 150, N_bltz);
    }

    /** Last step of 6) (i.e. bltz) -- Calculate the masses and momenta */
    class_call(background_bltz_init(ppr,pba),
               pba->error_message,
               errmsg);
    /* Complete set of parameters
       We must calculate M from omega or vice versa if one of them is missing.
       ...
       reflect the implicit normalization of the distribution function. */
    for (n=0; n < N_bltz; n++){
      if (pba->m_bltz_in_eV[n] != 0.0){
        /* Case of only mass or mass and Omega/omega: */
        pba->M_bltz[n] = pba->m_bltz_in_eV[n]/_k_B_*_eV_/pba->T_bltz[n]/pba->T_cmb;
        class_call(background_bltz_momenta(pba->q_bltz_bg[n],
                                           pba->w_bltz_bg[n],
                                           pba->q_size_bltz_bg[n],
                                           pba->M_bltz[n],
                                           pba->factor_bltz[n],
                                           0.,
                                           NULL,
                   ...
                   pba->error_message,
                   errmsg);
        if (pba->Omega0_bltz[n] == 0.0){
          pba->Omega0_bltz[n] = rho_bltz/pba->H0/pba->H0;
        }
        else{
          fnu_factor = (pba->H0*pba->H0*pba->Omega0_bltz[n]/rho_bltz);
          pba->factor_bltz[n] *= fnu_factor;
          /* dlnf0dlnq is already computed, but it is independent of any
             normalization of f0. We don't need the factor anymore, but we
             store it nevertheless */
          pba->deg_bltz[n] *=fnu_factor;
        }
      }
      else{
        /* Case of only Omega/omega: */
        class_call(background_bltz_M_from_Omega(ppr,pba,n),
                   pba->error_message,
                   errmsg);
        pba->m_bltz_in_eV[n] = _k_B_/_eV_*pba->T_bltz[n]*pba->M_bltz[n]*pba->T_cmb;
      }
      pba->Omega0_bltz_tot += pba->Omega0_bltz[n];
    }

  }
  class_test(pba->Omega0_bltz_tot<0,errmsg,"You cannot set the bltz density to negative values.");
  if (has_m_budget == _TRUE_) {
    class_test(Omega_m_remaining < pba->Omega0_bltz_tot, errmsg, "Too much energy density from massive species. At this point only %e is left for Omega_m, but requested 'Omega_bltz = %e' (summed over all species)",Omega_m_remaining, pba->Omega0_bltz_tot);
    Omega_m_remaining-= pba->Omega0_bltz_tot;
  }
  ...
  /* 8) ** ADDITIONAL SPECIES ** --> Add your species here */

  /** 8.1) Decaying DM into DR */
  /** 8.1.a) Omega_0_dcdmdr (DCDM, i.e. decaying CDM) */
  /* Read */
  ...
  /** 8.1.b) Omega_ini_dcdm or omega_ini_dcdm */
  /* Read */
  ...
  if (pba->Omega0_dcdmdr > 0 || (pba->Omega_ini_dcdm > 0.)) {
    ...
    /** 8.1.c) Gamma in same units as H0, i.e. km/(s Mpc)*/
    /* Read */
  ...
  /** 8.2) Multi-interacting dark matter (idm) */
  /** 8.2.1) Global parameters for all interacting Dark Matter components */

  /** 8.2.1.a) Amount of interacting dark matter*/
  ...
  /** 8.2.1.b) Mass of interacting dark matter*/
  /* Read */
  ...
  /** 8.2.2) Dark Matter interacting with Dark Radiation, ETHOS-parametrization/NADM parametrization, see explanatory.ini */

  /** 8.2.2.a) Amount of idr  */
  /* Read */
  ...
  if (pba->Omega0_idm > 0. || f_idm > 0) {
    ...
    /** 8.2.2.f) Strength of self interactions */
    class_read_double_one_of_two("b_dark","b_idr",pth->b_idr);

    /** 8.2.2.g) Read alpha_idm_dr or alpha_dark */
  ...
  /** 8.2.3) Dark Matter interacting with baryons */
  /** 8.2.3.a) idm_b coupling stregth */
  if (pba->Omega0_idm > 0 || f_idm > 0) {
  ...
  /** 8.2.4) Dark Matter interacting with photons */
  /** 8.2.4.a) idm_g coupling stregth */
  ...
  /** 8.3) Final consistency checks for dark matter species */
  ...
  if (pba->Omega0_fld != 0.) {
    ...
    if (pba->fluid_equation_of_state == EDE) {
      /** 9.a.2.3) Equation of state of the fluid in 'EDE' case */
      /* Read */
      class_read_double("w0_fld",pba->w0_fld);
...
int input_read_parameters_additional(struct file_content* pfc,
  ...
  */
  ...
  if (pba->N_bltz>0) {
    class_test(ppr->bltz_fluid_trigger_tau_over_tau_k==ppr->radiation_streaming_trigger_tau_over_tau_k,
               errmsg,
               "please choose different values for precision parameters bltz_fluid_trigger_tau_over_tau_k and radiation_streaming_trigger_tau_over_tau_k, in order to avoid switching two approximation schemes at the same time");

    class_test(ppr->bltz_fluid_trigger_tau_over_tau_k==ppr->ur_fluid_trigger_tau_over_tau_k,
               errmsg,
               "please choose different values for precision parameters bltz_fluid_trigger_tau_over_tau_k and ur_fluid_trigger_tau_over_tau_k, in order to avoid switching two approximation schemes at the same time");
  }
  ...
  if (pba->Omega0_idr != 0.){
    ...
    class_test(ppr->idr_streaming_trigger_tau_over_tau_k==ppr->bltz_fluid_trigger_tau_over_tau_k,
               errmsg,
               "please choose different values for precision parameters dark_radiation_trigger_tau_over_tau_k and bltz_fluid_trigger_tau_over_tau_k, in order to avoid switching two approximation schemes at the same time");
  }
...
int input_default_params(struct background *pba,
  ...
  pba->bltz_psd_files = NULL;
  /** 6.c) Analytic distribution function */
  pba->bltz_psd_parameters = NULL;
  pba->Omega0_bltz_tot = 0.;
  /** 6.d) --> See read_parameters_background */
  /** 6.e) bltz temperature */
  pba->T_bltz_default = 0.71611; /* this value gives m/omega = 93.14 eV b*/
  pba->T_bltz = NULL;
  /** 6.f) bltz chemical potential */
  pba->ksi_bltz_default = 0.;
  pba->ksi_bltz = NULL;
  /** 6.g) bltz degeneracy parameter */
  pba->deg_bltz_default = 1.;
  pba->deg_bltz = NULL;
  ...
  /** 8.1) Decaying CDM into Dark Radiation = dcdm+dr */
  /** 8.1.a) Current fractional density of dcdm+dr */
  pba->Omega0_dcdmdr = 0.0;
  pba->Omega0_dcdm = 0.0;
  /** 8.1.b) Initial fractional density of dcdm+dr */
  pba->Omega_ini_dcdm = 0.;
  /** 8.1.c) Decay constant */
  pba->Gamma_dcdm = 0.0;
  pba->tau_dcdm = 0.0;

  /** 8.2) Interacting Dark Matter */
  /** 8.2.1.a) Current factional density of idm */
  pba->Omega0_idm = 0;
  /** 8.2.1.a) Mass of idm in eV*/
  pth->m_idm = 1.e9;
  /** 8.2.2) Current fractional density of idr */
  pba->Omega0_idr = 0.0;
  pba->T_idr = 0.0;
  /** 8.2.2.c) Coupling idm_dr*/
  pth->a_idm_dr = 0.;
  /** 8.2.2.d) temperature scaling idm_dr */
  pth->n_index_idm_dr = 0;
  /** 8.2.2.e) Approximation mode of idr */
  ppt->idr_nature=idr_free_streaming;
  /** 8.2.2.f) idr self-interactions*/
  pth->b_idr = 0.;
  /** 8.2.2.g, 7.2.2.h)  angular coefficients */
  ppt->alpha_idm_dr = NULL;
  ppt->beta_idr = NULL;
  /** 8.2.3.a) idm_b coupling */
  pth->cross_idm_b = 0.;
  /** 8.2.3.b) temperature scaling idm_b */
  pth->n_index_idm_b = 0.;
  pth->n_coeff_idm_b = 0.;
  /** 8.2.4.a) idm_g coupling */
  pth->cross_idm_g = 0.;
  pth->u_idm_g = 0.;
  /** 8.2.4.b) temperature scaling idm_g */
  pth->n_index_idm_g = 0;
  ...
  pba->Omega0_fld = 0.;
  pba->Omega0_scf = 0.;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_bltz_tot-pba->Omega0_dcdmdr - pba->Omega0_idr -pba->Omega0_idm;
  /** 9.a) Omega fluid */
  /** 9.a.1) PPF approximation */
  pba->use_ppf = _TRUE_;
