    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          if (pba->has_bltz == _TRUE_){
            for (n_bltz = 0; n_bltz < pba->N_bltz; n_bltz++){
              class_store_double(dataptr,tk[ppt->index_tp_delta_bltz1+n_bltz],ppt->has_source_delta_bltz,storeidx);
            }
          }
        ...
        if (ppt->has_velocity_transfers == _TRUE_) {
          ...
          if (pba->has_bltz == _TRUE_){
            for (n_bltz = 0; n_bltz < pba->N_bltz; n_bltz++){
              class_store_double(dataptr,tk[ppt->index_tp_theta_bltz1+n_bltz],ppt->has_source_theta_bltz,storeidx);
            }
          }
        ...
        class_store_double_or_default(dataptr,-tk[ppt->index_tp_delta_bltz1]/k2,ppt->has_source_delta_bltz,storeidx,0.0);
...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    class_store_columntitle(titles,"-T_bltz/k2",_TRUE_);
...
int perturbations_init(
  ...
  if (pba->has_bltz == _TRUE_) {
    ...
    class_test ((ppr->bltz_fluid_approximation < bltzfa_mb) ||
                (ppr->bltz_fluid_approximation > bltzfa_none),
                ppt->error_message,
                "your bltz_fluid_approximation is set to %d, out of range defined in perturbations.h",ppr->bltz_fluid_approximation);

    if (ppt->has_nc_density == _TRUE_) { // BLTZ_MOD: I don't know if this is important.
      if (ppt->perturbations_verbose > 0) {
        fprintf(stdout," -> [WARNING:] You request the number count Cl's in presence of non-cold dark matter.\n    Like in all previous CLASS and CLASSgal versions, this will be inferred from the total matter density,\n    but it could make much more sense physically to compute it from the CDM+baryon density only.\n    To get the latter behavior you would just need to change one line in transfer.c:\n    search there for a comment starting with 'use here delta_cb'\n");
      }
    }
  ...
  if (ppt->has_tensors == _TRUE_) {
    ...
    ppt->evolve_tensor_bltz = _FALSE_;
    ...
    case (tm_massless_approximation):
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_bltz == _TRUE_))
...
int perturbations_indices(
  ...
  ppt->has_source_delta_bltz = _FALSE_;
  ppt->has_source_delta_tot = _FALSE_;
  ...
  ppt->has_source_theta_bltz = _FALSE_;
  ppt->has_source_theta_tot = _FALSE_;
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_bltz == _TRUE_)
          ppt->has_source_delta_bltz = _TRUE_;
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if (pba->has_bltz == _TRUE_)
          ppt->has_source_theta_bltz = _TRUE_;
      ...
      class_define_index(ppt->index_tp_delta_bltz1,ppt->has_source_delta_bltz,index_type,pba->N_bltz);
      ...
      class_define_index(ppt->index_tp_theta_bltz1,ppt->has_source_theta_bltz,index_type,pba->N_bltz);
...
int perturbations_workspace_init(
  ...
  if (_scalars_) {
    ...
    if (pba->has_bltz == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_bltz);
  ...
  if (_tensors_) {
    ...
    if (pba->has_ur == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_ur);
    if (pba->has_ncdm == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_ncdm);
    if (pba->has_bltz == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_bltz); // BLTZ_MOD: Should I change this?
  }
  ...
  if (_scalars_) {
    ...
    class_define_index(ppw->index_ap_bltzfa,pba->has_bltz,index_ap,1);
  ...
  if (_scalars_) {
    ...
    if (pba->has_bltz == _TRUE_) {
      ppw->approx[ppw->index_ap_bltzfa]=(int)bltzfa_off;
    }
  }
  ...
  if (_scalars_) {
    ...
    if ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
      ...
      // BLTZ_MOD: This is a bit weird, I don't understand if this should be in here.
      class_alloc(ppw->delta_bltz,pba->N_bltz*sizeof(double),ppt->error_message);
      class_alloc(ppw->theta_bltz,pba->N_bltz*sizeof(double),ppt->error_message);
      class_alloc(ppw->shear_bltz,pba->N_bltz*sizeof(double),ppt->error_message);
...
int perturbations_workspace_free (
  ...
  if (_scalars_) {
    ...
    if ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
      ...
      // BLTZ_MOD: Related to the previous doubt, I don't know if this must be in here or not.
      free(ppw->delta_bltz);
      free(ppw->theta_bltz);
      free(ppw->shear_bltz);
...
int perturbations_solve(
  ...
  int n_ncdm,n_bltz,is_early_enough;
  ...
  if (pba->has_bltz == _TRUE_) {
    for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++) {
      class_test(fabs(ppw->pvecback[pba->index_bg_p_bltz1+n_bltz]/ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]-1./3.)>ppr->tol_bltz_initial_w,
                 ppt->error_message,
                 "your choice of initial time for integrating wavenumbers is inappropriate: it corresponds to a time at which the bltz species number %d is not ultra-relativistic anymore, with w=%g, p=%g and rho=%g\n",
                 n_bltz,
                 ppw->pvecback[pba->index_bg_p_bltz1+n_bltz]/ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz],
                 ppw->pvecback[pba->index_bg_p_bltz1+n_bltz],
                 ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]);
    }
  }
  ...
  while ((tau_upper - tau_lower)/tau_lower > ppr->tol_tau_approx) {
    ...
    if (pba->has_bltz == _TRUE_) {
      for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++) {
        if (fabs(ppw->pvecback[pba->index_bg_p_bltz1+n_bltz]/ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]-1./3.) > ppr->tol_bltz_initial_w)
          is_early_enough = _FALSE_;
      }
    }
...
int perturbations_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      if ((pba->has_bltz == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
        for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
          sprintf(tmp,"delta_bltz[%d]",n_bltz);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          sprintf(tmp,"theta_bltz[%d]",n_bltz);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          sprintf(tmp,"shear_bltz[%d]",n_bltz);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          sprintf(tmp,"cs2_bltz[%d]",n_bltz);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
        }
      }
    ...
    if (ppt->has_tensors == _TRUE_){
      ...
      if (ppt->evolve_tensor_bltz == _TRUE_) {
        for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
          sprintf(tmp,"delta_bltz[%d]",n_bltz);
          class_store_columntitle(ppt->tensor_titles,tmp,_TRUE_);
          sprintf(tmp,"theta_bltz[%d]",n_bltz);
    ...
    for (index_switch=1; index_switch<interval_number; index_switch++) {
      ...
      if (ppt->perturbations_verbose>2) {
        ...
        if (_scalars_) {
          ...
          if (pba->has_bltz == _TRUE_) {
            if ((interval_approx[index_switch-1][ppw->index_ap_bltzfa]==(int)bltzfa_off) &&
                (interval_approx[index_switch][ppw->index_ap_bltzfa]==(int)bltzfa_on)) {
              fprintf(stdout,"Mode k=%e: will switch on bltz fluid approximation at tau=%e\n",k,interval_limit[index_switch]);
            }
          }
        }
...
int perturbations_vector_init(
  ...
  int index_pt;
  int l;
  int n_ncdm,n_bltz,index_q,ncdm_l_size,bltz_l_size; //BLTZ_MOD: Do we need another index_q specific for bltz?
  double rho_plus_p_ncdm,rho_plus_p_bltz,q,q2,epsilon,a,factor;
  ...
  if (_scalars_) {
    ...
    if (pba->has_bltz == _TRUE_) {
      ppv->index_pt_psi0_bltz1 = index_pt; /* density of ultra-relativistic neutrinos/relics */
      ppv->N_bltz = pba->N_bltz;
      class_alloc(ppv->l_max_bltz,ppv->N_bltz*sizeof(double),ppt->error_message);
      class_alloc(ppv->q_size_bltz,ppv->N_bltz*sizeof(double),ppt->error_message);

      for (n_bltz = 0; n_bltz < pba->N_bltz; n_bltz++){
        // Set value of ppv->l_max_bltz:
        if (ppw->approx[ppw->index_ap_bltzfa] == (int)bltzfa_off){
          /* reject inconsistent values of the number of mutipoles in ultra relativistic neutrino hierarchy */
          class_test(ppr->l_max_bltz < 4,
                     ppt->error_message,
                     "ppr->l_max_bltz=%d should be at least 4, i.e. we must integrate at least over first four momenta of non-cold dark matter perturbed phase-space distribution",n_bltz);
          //Copy value from precision parameter:
          ppv->l_max_bltz[n_bltz] = ppr->l_max_bltz;
          ppv->q_size_bltz[n_bltz] = pba->q_size_bltz[n_bltz];
        }
        else{
          // In the fluid approximation, hierarchy is cut at lmax = 2 and q dependence is integrated out:
          ppv->l_max_bltz[n_bltz] = 2;
          ppv->q_size_bltz[n_bltz] = 1;
        }
        index_pt += (ppv->l_max_bltz[n_bltz]+1)*ppv->q_size_bltz[n_bltz];
      }
    }
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_bltz == _TRUE_) {
      ppv->index_pt_psi0_bltz1 = index_pt;
      ppv->N_bltz = pba->N_bltz;
      class_alloc(ppv->l_max_bltz,ppv->N_bltz*sizeof(double),ppt->error_message);
      class_alloc(ppv->q_size_bltz,ppv->N_bltz*sizeof(double),ppt->error_message);
  ...
  if (_scalars_) {
    ...
    if (pba->has_bltz == _TRUE_) {
      ...
      /* we don't need bltz multipoles above l=2 (but they are
         defined only when bltzfa is off) */

      index_pt = ppv->index_pt_psi0_bltz1;
      for (n_bltz = 0; n_bltz < ppv-> N_bltz; n_bltz++){
        for (index_q=0; index_q < ppv->q_size_bltz[n_bltz]; index_q++){
          for (l=0; l<=ppv->l_max_bltz[n_bltz]; l++){
            if (l>2) ppv->used_in_sources[index_pt]=_FALSE_;
            index_pt++;
          }
        }
  ...
  if (pa_old == NULL) {
    ...
    if (_scalars_) {
      ...
      if (pba->has_bltz == _TRUE_) {
        ...
        class_test(ppw->approx[ppw->index_ap_bltzfa] == (int)bltzfa_on,
                   ppt->error_message,
                   "scalar initial conditions assume bltz fluid approximation turned off");
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if ((pa_old[ppw->index_ap_tca] == (int)tca_on) && (ppw->approx[ppw->index_ap_tca] == (int)tca_off)) {
        ...
        if (pba->has_bltz == _TRUE_) {
          ...
          for (n_bltz = 0; n_bltz < ppv->N_bltz; n_bltz++){
            for (index_q=0; index_q < ppv->q_size_bltz[n_bltz]; index_q++){
              for (l=0; l<=ppv->l_max_bltz[n_bltz];l++){
                // This is correct with or without bltzfa, since ppv->lmax_bltz is set accordingly.
                ppv->y[ppv->index_pt_psi0_bltz1+index_pt] =
                  ppw->pv->y[ppw->pv->index_pt_psi0_bltz1+index_pt];
                index_pt++;
              }
            }
          }
        ...
        if ((pa_old[ppw->index_ap_ufa] == (int)ufa_off) && (ppw->approx[ppw->index_ap_ufa] == (int)ufa_on)) {
          ...
          if (pba->has_bltz == _TRUE_) {
            ...
            for (n_bltz = 0; n_bltz < ppv->N_bltz; n_bltz++){
              for (index_q=0; index_q < ppv->q_size_bltz[n_bltz]; index_q++){
                for (l=0; l<=ppv->l_max_bltz[n_bltz]; l++){
                  /* This is correct even when bltzfa == off, since ppv->l_max_bltz and
                     ppv->q_size_bltz is updated.*/
                  ppv->y[ppv->index_pt_psi0_bltz1+index_pt] =
                    ppw->pv->y[ppw->pv->index_pt_psi0_bltz1+index_pt];
                  index_pt++;
                }
     ...
     /* -- case of switching on bltz fluid
         approximation. Provide correct initial conditions to new set
         of variables */

      if (pba->has_bltz == _TRUE_) {

        if ((pa_old[ppw->index_ap_bltzfa] == (int)bltzfa_off) && (ppw->approx[ppw->index_ap_bltzfa] == (int)bltzfa_on)) {

          if (ppt->perturbations_verbose>2)
            fprintf(stdout,"Mode k=%e: switch on bltz fluid approximation at tau=%e\n",k,tau);
          ...
          a = ppw->pvecback[pba->index_bg_a];
          index_pt = ppw->pv->index_pt_psi0_bltz1;
          for (n_bltz = 0; n_bltz < ppv->N_bltz; n_bltz++){
            // We are in the fluid approximation, so bltz_l_size is always 3.
            bltz_l_size = ppv->l_max_bltz[n_bltz]+1;
            rho_plus_p_bltz = ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]+
              ppw->pvecback[pba->index_bg_p_bltz1+n_bltz];
            for (l=0; l<=2; l++){
              ppv->y[ppv->index_pt_psi0_bltz1+bltz_l_size*n_bltz+l] = 0.0;
            }
            factor = pba->factor_bltz[n_bltz]/pow(a,4);
            for (index_q=0; index_q < ppw->pv->q_size_bltz[n_bltz]; index_q++){
              // Integrate over distributions:
              q = pba->q_bltz[n_bltz][index_q];
              q2 = q*q;
              epsilon = sqrt(q2+a*a*pba->M_bltz[n_bltz]*pba->M_bltz[n_bltz]);
              ppv->y[ppv->index_pt_psi0_bltz1+bltz_l_size*n_bltz] +=
                pba->w_bltz[n_bltz][index_q]*q2*epsilon*
                ppw->pv->y[index_pt];

              ppv->y[ppv->index_pt_psi0_bltz1+bltz_l_size*n_bltz+1] +=
                pba->w_bltz[n_bltz][index_q]*q2*q*
                ppw->pv->y[index_pt+1];

              ppv->y[ppv->index_pt_psi0_bltz1+bltz_l_size*n_bltz+2] +=
                pba->w_bltz[n_bltz][index_q]*q2*q2/epsilon*
                ppw->pv->y[index_pt+2];

              //Jump to next momentum bin in ppw->pv->y:
              index_pt += (ppw->pv->l_max_bltz[n_bltz]+1);
            }
            ppv->y[ppv->index_pt_psi0_bltz1+bltz_l_size*n_bltz] *=factor/ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz];
            ppv->y[ppv->index_pt_psi0_bltz1+bltz_l_size*n_bltz+1] *=k*factor/rho_plus_p_bltz;
            ppv->y[ppv->index_pt_psi0_bltz1+bltz_l_size*n_bltz+2] *=2.0/3.0*factor/rho_plus_p_bltz;
          }
        }
      }
...
int perturbations_vector_free(
  ...
  if (pv->l_max_bltz != NULL) free(pv->l_max_bltz);
  if (pv->q_size_bltz != NULL) free(pv->q_size_bltz);  
  free(pv->y);
  free(pv->dy);
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  if (_scalars_) {
    ...
    if (pba->has_bltz == _TRUE_) {
      for (n_bltz=0; n_bltz<pba->N_bltz; n_bltz++){
        rho_r += ppw->pvecback[pba->index_bg_rho_bltz1 + n_bltz];
        rho_nu += ppw->pvecback[pba->index_bg_rho_bltz1 + n_bltz];
      }
    }
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      /* all relativistic relics: ur, early ncdm, dr, bltz */

      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_bltz == _TRUE_) || (pba->has_dr == _TRUE_) || (pba->has_idr == _TRUE_)) {
    ...
    if ((ppt->has_cdi == _TRUE_) && (index_ic == ppt->index_ic_cdi)) {
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_bltz == _TRUE_)) { // BLTZ_MOD: Did I do this right?
    ...
    if ((ppt->has_nid == _TRUE_) && (index_ic == ppt->index_ic_nid)) {
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_ncdm == _FALSE_) && (pba->has_bltz == _FALSE_),
                 ppt->error_message,
                 "not consistent to ask for NID in absence of ur, ncdm or bltz species!");
    ...
    if ((ppt->has_niv == _TRUE_) && (index_ic == ppt->index_ic_niv)) {
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_ncdm == _FALSE_) && (pba->has_ncdm == _FALSE_),
                 ppt->error_message,
                 "not consistent to ask for NIV in absence of ur, ncdm or bltz species!");
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      /** - (e) In any gauge, we should now implement the relativistic initial conditions in ur, bltz and ncdm variables */
    ...
    if (pba->has_bltz == _TRUE_) {
      idx = ppw->pv->index_pt_psi0_bltz1;
      for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
        ...
        for (index_q=0; index_q < ppw->pv->q_size_bltz[n_bltz]; index_q++) {
          ...
          epsilon = sqrt(q*q+a*a*pba->M_bltz[n_bltz]*pba->M_bltz[n_bltz]);

          ppw->pv->y[idx] = -0.25 * delta_ur * pba->dlnf0_dlnq_bltz[n_bltz][index_q];

          ppw->pv->y[idx+1] =  -epsilon/3./q/k*theta_ur* pba->dlnf0_dlnq_bltz[n_bltz][index_q];

          ppw->pv->y[idx+2] = -0.5 * shear_ur * pba->dlnf0_dlnq_bltz[n_bltz][index_q];

          ppw->pv->y[idx+3] = -0.25 * l3_ur * pba->dlnf0_dlnq_bltz[n_bltz][index_q];

          //Jump to next momentum bin:
          idx += (ppw->pv->l_max_bltz[n_bltz]+1);
...
int perturbations_approximations(
  ...
  if (_scalars_) {
    ...
    if (pba->has_bltz == _TRUE_) {
      ...
      if ((tau/tau_k > ppr->bltz_fluid_trigger_tau_over_tau_k) &&
          (ppr->bltz_fluid_approximation != bltzfa_none)) {

        ppw->approx[ppw->index_ap_bltzfa] = (int)bltzfa_on;
      }
      else {
        ppw->approx[ppw->index_ap_bltzfa] = (int)bltzfa_off;
      }
...
int perturbations_timescale(
  ...
  if ((ppt->has_scalars == _TRUE_) && (pppaw->index_md == ppt->index_md_scalars)) {
    ...
    *timescale = tau_h;
    ...
    if ((ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) || (pba->has_ncdm == _TRUE_) || (pba->has_bltz == _TRUE_)) // BLTZ_MOD: Did I do this right?
...
int perturbations_total_stress_energy(
  ...
  double w_fld,dw_over_da_fld,integral_fld;
  double gwncdm;
  double rho_delta_bltz=0.;
  double rho_plus_p_theta_bltz=0.;
  double rho_plus_p_shear_bltz=0.;
  double delta_p_bltz=0.;
  double rho_plus_p_bltz;
  int n_bltz;
  double cg2_bltz,w_bltz,rho_bltz_bg,p_bltz_bg,pseudo_p_bltz;
  double gwbltz;
  ...
  if (_scalars_) {
    ...
    /* infer delta_cb abd theta_cb (perturbations from CDM and baryons) before adding ncdm or bltz */
    ...
    if (pba->has_bltz == _TRUE_) {
      idx = ppw->pv->index_pt_psi0_bltz1;
      if (ppw->approx[ppw->index_ap_bltzfa] == (int)bltzfa_on){
        // The perturbations are evolved integrated:
        for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
          rho_bltz_bg = ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz];
          p_bltz_bg = ppw->pvecback[pba->index_bg_p_bltz1+n_bltz];
          pseudo_p_bltz = ppw->pvecback[pba->index_bg_pseudo_p_bltz1+n_bltz];

          rho_plus_p_bltz = rho_bltz_bg + p_bltz_bg;
          w_bltz = p_bltz_bg/rho_bltz_bg;
          cg2_bltz = w_bltz*(1.0-1.0/(3.0+3.0*w_bltz)*(3.0*w_bltz-2.0+pseudo_p_bltz/p_bltz_bg));
          if ((ppt->has_source_delta_bltz == _TRUE_) || (ppt->has_source_theta_bltz == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
            ppw->delta_bltz[n_bltz] = y[idx];
            ppw->theta_bltz[n_bltz] = y[idx+1];
            ppw->shear_bltz[n_bltz] = y[idx+2];
          }

          ppw->delta_rho += rho_bltz_bg*y[idx];
          ppw->rho_plus_p_theta += rho_plus_p_bltz*y[idx+1];
          ppw->rho_plus_p_shear += rho_plus_p_bltz*y[idx+2];
          ppw->delta_p += cg2_bltz*rho_bltz_bg*y[idx];

          ppw->rho_plus_p_tot += rho_plus_p_bltz;
        ...
        for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
          rho_delta_bltz = 0.0;
          rho_plus_p_theta_bltz = 0.0;
          rho_plus_p_shear_bltz = 0.0;
          delta_p_bltz = 0.0;
          factor = pba->factor_bltz[n_bltz]/pow(a,4);
          ...
          for (index_q=0; index_q < ppw->pv->q_size_bltz[n_bltz]; index_q ++) {
            ...
            q = pba->q_bltz[n_bltz][index_q];
            q2 = q*q;
            epsilon = sqrt(q2+pba->M_bltz[n_bltz]*pba->M_bltz[n_bltz]*a2);

            rho_delta_bltz += q2*epsilon*pba->w_bltz[n_bltz][index_q]*y[idx];
            rho_plus_p_theta_bltz += q2*q*pba->w_bltz[n_bltz][index_q]*y[idx+1];
            rho_plus_p_shear_bltz += q2*q2/epsilon*pba->w_bltz[n_bltz][index_q]*y[idx+2];
            delta_p_bltz += q2*q2/epsilon*pba->w_bltz[n_bltz][index_q]*y[idx];

            //Jump to next momentum bin:
            idx+=(ppw->pv->l_max_bltz[n_bltz]+1);
          }

          rho_delta_bltz *= factor;
          rho_plus_p_theta_bltz *= k*factor;
          rho_plus_p_shear_bltz *= 2.0/3.0*factor;
          delta_p_bltz *= factor/3.;

          if ((ppt->has_source_delta_bltz == _TRUE_) || (ppt->has_source_theta_bltz == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
            ppw->delta_bltz[n_bltz] = rho_delta_bltz/ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz];
            ppw->theta_bltz[n_bltz] = rho_plus_p_theta_bltz/
              (ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]+ppw->pvecback[pba->index_bg_p_bltz1+n_bltz]);
            ppw->shear_bltz[n_bltz] = rho_plus_p_shear_bltz/
              (ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]+ppw->pvecback[pba->index_bg_p_bltz1+n_bltz]);
          }

          ppw->delta_rho += rho_delta_bltz;
          ppw->rho_plus_p_theta += rho_plus_p_theta_bltz;
          ppw->rho_plus_p_shear += rho_plus_p_shear_bltz;
          ppw->delta_p += delta_p_bltz;

          ppw->rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]+ppw->pvecback[pba->index_bg_p_bltz1+n_bltz];
        }
      }
      if (ppt->has_source_delta_m == _TRUE_) {
        for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
          delta_rho_m += ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]*ppw->delta_bltz[n_bltz]; // contribution to delta rho_matter
          rho_m += ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz];
        }
      }
      if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
        for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
          rho_plus_p_theta_m += (ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]+ppw->pvecback[pba->index_bg_p_bltz1+n_bltz])
            *ppw->theta_bltz[n_bltz]; // contribution to [(rho+p)theta]_matter
          rho_plus_p_m += (ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]+ppw->pvecback[pba->index_bg_p_bltz1+n_bltz]);
        }
      }
    }
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_ur == _TRUE_){
      ...
      if (ppt->tensor_method == tm_massless_approximation) {
        ...
        if (pba->has_bltz == _TRUE_) {
          for (n_bltz = 0; n_bltz < pba->N_bltz; n_bltz++) {
            /* (3 p_bltz1) is the "relativistic" contribution to rho_bltz1 */
            rho_relativistic += 3.*ppw->pvecback[pba->index_bg_p_bltz1+n_bltz];
          }
        }
      }
    ...
    /** - --> bltz contribution to gravitational wave source: */
    if (ppt->evolve_tensor_bltz == _TRUE_){
      ...
      for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
        ...
        for (index_q=0; index_q < ppw->pv->q_size_bltz[n_bltz]; index_q ++) {
          ...
          gwbltz += q2*q2/epsilon*pba->w_bltz[n_bltz][index_q]*(1./15.*y[idx]+2./21.*y[idx+2]+1./35.*y[idx+4]);

          //Jump to next momentum bin:
          idx+=(ppw->pv->l_max_bltz[n_bltz]+1);
        }

        gwbltz *= -_SQRT6_*4*a2*factor;

        ppw->gw_source += gwbltz;
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    /* delta_bltz1 */
    if (ppt->has_source_delta_bltz == _TRUE_) {
      for (index_tp = ppt->index_tp_delta_bltz1; index_tp < ppt->index_tp_delta_bltz1+pba->N_bltz; index_tp++) {
        _set_source_(index_tp) = ppw->delta_bltz[index_tp - ppt->index_tp_delta_bltz1]
          + 3.*a_prime_over_a*(1+pvecback[index_tp - ppt->index_tp_delta_bltz1 + pba->index_bg_p_bltz1]
                               /pvecback[index_tp - ppt->index_tp_delta_bltz1 + pba->index_bg_rho_bltz1])*theta_over_k2; // N-body gauge correction
      }
    }
    ...
    /* theta_bltz1 */
    if (ppt->has_source_theta_bltz == _TRUE_) {
      for (index_tp = ppt->index_tp_theta_bltz1; index_tp < ppt->index_tp_theta_bltz1+pba->N_bltz; index_tp++) {
        _set_source_(index_tp) = ppw->theta_bltz[index_tp - ppt->index_tp_theta_bltz1]
          + theta_shift; // N-body gauge correction
      }
    }
  }
...
int perturbations_print_variables(double tau,
  ...
  /** - bltz sector begins */
  int n_bltz;
  double *delta_bltz=NULL, *theta_bltz=NULL, *shear_bltz=NULL, *delta_p_over_delta_rho_bltz=NULL;
  double rho_bltz_bg, p_bltz_bg, pseudo_p_bltz, w_bltz;
  double rho_delta_bltz = 0.0;
  double rho_plus_p_theta_bltz = 0.0;
  double rho_plus_p_shear_bltz = 0.0;
  double delta_p_bltz = 0.0;
  /** - bltz sector ends */
  ...
  if (pba->has_bltz == _TRUE_){
    class_alloc(delta_bltz, sizeof(double)*pba->N_bltz,error_message);
    class_alloc(theta_bltz, sizeof(double)*pba->N_bltz,error_message);
    class_alloc(shear_bltz, sizeof(double)*pba->N_bltz,error_message);
    class_alloc(delta_p_over_delta_rho_bltz, sizeof(double)*pba->N_bltz,error_message);
  }
  ...
  if (_scalars_) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      if (ppw->approx[ppw->index_ap_bltzfa] == (int)bltzfa_on){
        // The perturbations are evolved integrated:
        for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
          rho_bltz_bg = pvecback[pba->index_bg_rho_bltz1+n_bltz];
          p_bltz_bg = pvecback[pba->index_bg_p_bltz1+n_bltz];
          pseudo_p_bltz = pvecback[pba->index_bg_pseudo_p_bltz1+n_bltz];
          w_bltz = p_bltz_bg/rho_bltz_bg;

          delta_bltz[n_bltz] = y[idx];
          theta_bltz[n_bltz] = y[idx+1];
          shear_bltz[n_bltz] = y[idx+2];
          //This is the adiabatic sound speed:
          delta_p_over_delta_rho_bltz[n_bltz] = w_bltz*(1.0-1.0/(3.0+3.0*w_bltz)*(3.0*w_bltz-2.0+pseudo_p_bltz/p_bltz_bg));
          idx += ppw->pv->l_max_bltz[n_bltz]+1;
        }
      }
      else{
        ...
        for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
          ...
          delta_bltz[n_bltz] = rho_delta_bltz/ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz];
          theta_bltz[n_bltz] = rho_plus_p_theta_bltz/
            (ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]+ppw->pvecback[pba->index_bg_p_bltz1+n_bltz]);
          shear_bltz[n_bltz] = rho_plus_p_shear_bltz/
            (ppw->pvecback[pba->index_bg_rho_bltz1+n_bltz]+ppw->pvecback[pba->index_bg_p_bltz1+n_bltz]);
          delta_p_over_delta_rho_bltz[n_bltz] = delta_p_bltz/rho_delta_bltz;
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_bltz == _TRUE_) {
        for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
          /** - --> TODO: gauge transformation of delta, deltaP/rho (?) and theta using -= 3aH(1+w_bltz) alpha for delta. */
        }
      }
    ...
    if ((pba->has_bltz == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
      for (n_bltz=0; n_bltz < pba->N_bltz; n_bltz++){
        class_store_double(dataptr, delta_bltz[n_bltz], _TRUE_, storeidx);
        class_store_double(dataptr, theta_bltz[n_bltz], _TRUE_, storeidx);
        class_store_double(dataptr, shear_bltz[n_bltz], _TRUE_, storeidx);
        class_store_double(dataptr, delta_p_over_delta_rho_bltz[n_bltz],  _TRUE_, storeidx);
      }
    }
  ...
  if (pba->has_bltz == _TRUE_){
    ...
    free(theta_bltz);
    free(shear_bltz);
    free(delta_p_over_delta_rho_bltz);
  }
...
int perturbations_derivs(double tau,
  ...
  /* for use with non-cold dark matter with generalized Boltzmann hierarchy (bltz): */
  int n_bltz;
  double rho_bltz_bg,p_bltz_bg,pseudo_p_bltz,w_bltz,ca2_bltz,ceff2_bltz=0.,cvis2_bltz=0.;
  ...
  if (_scalars_) {
    ...
    /** - ---> non-cold dark matter with generalized Boltzmann hierarchy (bltz): massive neutrinos, WDM, etc. */
    //TBC: curvature in all bltz
    if (pba->has_bltz == _TRUE_) {

      idx = pv->index_pt_psi0_bltz1;

      /** - ----> first case: use a fluid approximation (bltzfa) */
      //TBC: curvature
      if (ppw->approx[ppw->index_ap_bltzfa] == (int)bltzfa_on) {

        /** - -----> loop over species */

        for (n_bltz=0; n_bltz<pv->N_bltz; n_bltz++) {

          /** - -----> define intermediate quantitites */

          rho_bltz_bg = pvecback[pba->index_bg_rho_bltz1+n_bltz]; /* background density */
          p_bltz_bg = pvecback[pba->index_bg_p_bltz1+n_bltz]; /* background pressure */
          pseudo_p_bltz = pvecback[pba->index_bg_pseudo_p_bltz1+n_bltz]; /* pseudo-pressure (see CLASS IV paper) */
          w_bltz = p_bltz_bg/rho_bltz_bg; /* equation of state parameter */
          ca2_bltz = w_bltz/3.0/(1.0+w_bltz)*(5.0-pseudo_p_bltz/p_bltz_bg); /* adiabatic sound speed */
          ...
          /* different ansatz for sound speed c_eff and viscosity speed c_vis */
          if (ppr->bltz_fluid_approximation == bltzfa_mb) {
            ceff2_bltz = ca2_bltz;
            cvis2_bltz = 3.*w_bltz*ca2_bltz;
          }
          if (ppr->bltz_fluid_approximation == bltzfa_hu) {
            ceff2_bltz = ca2_bltz;
            cvis2_bltz = w_bltz;
          }
          if (ppr->bltz_fluid_approximation == bltzfa_CLASS) {
            ceff2_bltz = ca2_bltz;
            cvis2_bltz = 3.*w_bltz*ca2_bltz;
          }
          ...
          dy[idx] = -(1.0+w_bltz)*(y[idx+1]+metric_continuity)-
            3.0*a_prime_over_a*(ceff2_bltz-w_bltz)*y[idx];

          /** - -----> exact euler equation */

          dy[idx+1] = -a_prime_over_a*(1.0-3.0*ca2_bltz)*y[idx+1]+
            ceff2_bltz/(1.0+w_bltz)*k2*y[idx]-k2*y[idx+2]
            + metric_euler;
          ...
          if (ppr->bltz_fluid_approximation == bltzfa_mb) {
            ...
            dy[idx+2] = -3.0*(a_prime_over_a*(2./3.-ca2_bltz-pseudo_p_bltz/p_bltz_bg/3.)+1./tau)*y[idx+2]
              +8.0/3.0*cvis2_bltz/(1.0+w_bltz)*s_l[2]*(y[idx+1]+metric_shear);

          }

          if (ppr->bltz_fluid_approximation == bltzfa_hu) {

            dy[idx+2] = -3.0*a_prime_over_a*ca2_bltz/w_bltz*y[idx+2]
              +8.0/3.0*cvis2_bltz/(1.0+w_bltz)*s_l[2]*(y[idx+1]+metric_shear);
          ...
          if (ppr->bltz_fluid_approximation == bltzfa_CLASS) {
            ...
            dy[idx+2] = -3.0*(a_prime_over_a*(2./3.-ca2_bltz-pseudo_p_bltz/p_bltz_bg/3.)+1./tau)*y[idx+2]
              +8.0/3.0*cvis2_bltz/(1.0+w_bltz)*s_l[2]*(y[idx+1]+metric_ufa_class);

          }

          /** - -----> jump to next species */

          idx += pv->l_max_bltz[n_bltz]+1;
        }
      }
      ...
      /** - ----> second case: use exact equation (Boltzmann hierarchy on momentum grid) */
        ...
        for (n_bltz=0; n_bltz<pv->N_bltz; n_bltz++) {
          ...
          for (index_q=0; index_q < pv->q_size_bltz[n_bltz]; index_q++) {

            /** - -----> define intermediate quantities */

            dlnf0_dlnq = pba->dlnf0_dlnq_bltz[n_bltz][index_q];
            q = pba->q_bltz[n_bltz][index_q];
            epsilon = sqrt(q*q+a2*pba->M_bltz[n_bltz]*pba->M_bltz[n_bltz]);
            qk_div_epsilon = k*q/epsilon;

            /** - -----> bltz density for given momentum bin */

            dy[idx] = -qk_div_epsilon*y[idx+1]+metric_continuity*dlnf0_dlnq/3.;

            /** - -----> bltz velocity for given momentum bin */

            dy[idx+1] = qk_div_epsilon/3.0*(y[idx] - 2*s_l[2]*y[idx+2])
              -epsilon*metric_euler/(3*q*k)*dlnf0_dlnq;

            /** - -----> bltz shear for given momentum bin */

            dy[idx+2] = qk_div_epsilon/5.0*(2*s_l[2]*y[idx+1]-3.*s_l[3]*y[idx+3])
              -s_l[2]*metric_shear*2./15.*dlnf0_dlnq;

            /** - -----> bltz l>3 for given momentum bin */

            for (l=3; l<pv->l_max_bltz[n_bltz]; l++){
              dy[idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]);
            }

            /** - -----> bltz lmax for given momentum bin (truncation as in Ma and Bertschinger)
                but with curvature taken into account a la arXiv:1305.3261 */
    ...
    if (ppt->evolve_tensor_bltz == _TRUE_) {
      ...
      for (n_bltz=0; n_bltz<pv->N_bltz; n_bltz++) {
        ...
        for (index_q=0; index_q < pv->q_size_bltz[n_bltz]; index_q++) {
          ...
          /** - ----> bltz l>0 for given momentum bin */

          for (l=1; l<pv->l_max_bltz[n_bltz]; l++){
            dy[idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]);
          }
