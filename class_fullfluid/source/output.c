...
int output_tk(
  ...
  if (pop->output_format == camb_format) {
    ...
    class_test(pba->N_bltz>1,
               pop->error_message,
               "you wish to output the transfer functions in CMBFAST/CAMB format but you have more than one non-cold dark matter (bltz) species. The two are not compatible (since CMBFAST/CAMB only have one bltz species): switch to CLASS output format or keep only on bltz species");
