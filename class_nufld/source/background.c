 ...
 * * routines related to nufld written by T. Bertólez-Martínez in 2023
...
int background_free_input(
  ...
  if (pba->Omega0_nufld_tot != 0.) {
    for (k=0; k<pba->N_nufld; k++) {
      free(pba->q_nufld[k]);
      free(pba->w_nufld[k]);
      free(pba->q_nufld_bg[k]);
      free(pba->w_nufld_bg[k]);
      free(pba->dlnf0_dlnq_nufld[k]);
    }
    free(pba->nufld_quadrature_strategy);
    free(pba->nufld_input_q_size);
    free(pba->nufld_qmax);
    free(pba->q_nufld);
    free(pba->w_nufld);
    free(pba->q_nufld_bg);
    free(pba->w_nufld_bg);
    free(pba->dlnf0_dlnq_nufld);
    free(pba->q_size_nufld);
    free(pba->q_size_nufld_bg);
    free(pba->M_nufld);
    free(pba->T_nufld);
    free(pba->ksi_nufld);
    free(pba->deg_nufld);
    ...
    if (pba->nufld_psd_files!=NULL)
      free(pba->nufld_psd_files);
    if (pba->nufld_psd_parameters!=NULL)
      free(pba->nufld_psd_parameters);
  }
...
int background_indices(
  ...
  /* - indices for nufld. We only define the indices for nufld1
     (density, pressure, pseudo-pressure), the other nufld indices
     are contiguous */
...
/**
 * This is the routine where the distribution function f0(q) of each
 * nufld species is specified (it is the only place to modify if you
 * need a partlar f0(q))
 * nufld_MOD this is important: Where do we get the distribution function from?
 *
 * @param pbadist Input:  structure containing all parameters defining f0(q)
 * @param q       Input:  momentum
 * @param f0      Output: phase-space distribution
...
int background_nufld_distribution(
                                 void * pbadist,
                                 double q,
                                 double * f0
                                 ) {
  ...
  pbadist_local = pbadist;          /* restore actual format of pbadist */
  pba = pbadist_local->pba;         /* extract the background structure from it */
  param = pba->nufld_psd_parameters; /* extract the optional parameter list from it */
  n_nufld = pbadist_local->n_nufld;   /* extract index of nufld species under consideration */
  ksi = pba->ksi_nufld[n_nufld];      /* extract chemical potential */

  /** - shall we interpolate in file, or shall we use analytical formula below? */

  /** - a) deal first with the case of interpolating in files */
  if (pba->got_files[n_nufld]==_TRUE_) {
  ...
  /** - b) deal now with case of reading analytical function */
    ...
    /**
       Next enter your analytic expression(s) for the p.s.d.'s. If
       you need different p.s.d.'s for different species, put each
       p.s.d inside a condition, like for instance: if (n_nufld==2)
       {*f0=...}.  Remember that n_nufld = 0 refers to the first
       species.
    */
    ...
    *f0 = 1.0/pow(2*_PI_,3)*(1./(exp(q-ksi)+1.) +1./(exp(q+ksi)+1.));
    ...
    if (_FALSE_) {
      ...
      /* We must use the list of extra parameters read in input, stored in the
         nufld_psd_parameter list, extracted above from the structure
         and now called param[..] */

      /* check that this list has been read */
      class_test(param == NULL,
                 pba->error_message,
                 "Analytic expression wants to use 'nufld_psd_parameters', but they have not been entered!");
      ...
      *f0=0.0;
      for (i=0;i<3;i++) {
        ...
        *f0 += mixing_matrix[i][n_nufld]*1.0/pow(2*_PI_,3)*(1./(exp(q-pba->ksi_nufld[i])+1.) +1./(exp(q+pba->ksi_nufld[i])+1.));
...
int background_nufld_test_function(
                                  void * pbadist,
                                  double q,
                                  double * test
                                  ) {
...
/**
 * This function finds optimal quadrature weights for each nufld
 * species
 *
 * @param ppr Input: precision structure
 * @param pba Input/Output: background structure
...
int background_nufld_init(
  ...
  /* Allocate pointer arrays: */
  class_alloc(pba->q_nufld, sizeof(double*)*pba->N_nufld,pba->error_message);
  class_alloc(pba->w_nufld, sizeof(double*)*pba->N_nufld,pba->error_message);
  class_alloc(pba->q_nufld_bg, sizeof(double*)*pba->N_nufld,pba->error_message);
  class_alloc(pba->w_nufld_bg, sizeof(double*)*pba->N_nufld,pba->error_message);
  class_alloc(pba->dlnf0_dlnq_nufld, sizeof(double*)*pba->N_nufld,pba->error_message);

  /* Allocate pointers: */
  class_alloc(pba->q_size_nufld,sizeof(int)*pba->N_nufld,pba->error_message);
  class_alloc(pba->q_size_nufld_bg,sizeof(int)*pba->N_nufld,pba->error_message);
  class_alloc(pba->factor_nufld,sizeof(double)*pba->N_nufld,pba->error_message);

  for (k=0, filenum=0; k<pba->N_nufld; k++) {
    pbadist.n_nufld = k;
    pbadist.q = NULL;
    pbadist.tablesize = 0;
    /*Do we need to read in a file to interpolate the distribution function? */
    if ((pba->got_files!=NULL)&&(pba->got_files[k]==_TRUE_)) {
      psdfile = fopen(pba->nufld_psd_files+filenum*_ARGUMENT_LENGTH_MAX_,"r");
      class_test(psdfile == NULL,pba->error_message,
                 "Could not open file %s!",pba->nufld_psd_files+filenum*_ARGUMENT_LENGTH_MAX_);
      // Find size of table:
      for (row=0,status=2; status==2; row++) {
        status = fscanf(psdfile,"%lf %lf",&tmp1,&tmp2);
      }
    ...
    /* Handle perturbation qsampling: */
    if (pba->nufld_quadrature_strategy[k]==qm_auto) {
      /** Automatic q-sampling for this species */
      class_alloc(pba->q_nufld[k],_QUADRATURE_MAX_*sizeof(double),pba->error_message);
      class_alloc(pba->w_nufld[k],_QUADRATURE_MAX_*sizeof(double),pba->error_message);

      class_call(get_qsampling(pba->q_nufld[k],
                               pba->w_nufld[k],
                               &(pba->q_size_nufld[k]),
                               _QUADRATURE_MAX_,
                               ppr->tol_nufld,
                               pbadist.q,
                               pbadist.tablesize,
                               background_nufld_test_function,
                               background_nufld_distribution,
                               &pbadist,
                               pba->error_message),
                 pba->error_message,
                 pba->error_message);
      pba->q_nufld[k]=realloc(pba->q_nufld[k],pba->q_size_nufld[k]*sizeof(double));
      pba->w_nufld[k]=realloc(pba->w_nufld[k],pba->q_size_nufld[k]*sizeof(double));


      if (pba->background_verbose > 0) {
        printf("nufld species i=%d sampled with %d points for purpose of perturbation integration\n",
               k+1,
               pba->q_size_nufld[k]);
      }

      /* Handle background q_sampling: */
      class_alloc(pba->q_nufld_bg[k],_QUADRATURE_MAX_BG_*sizeof(double),pba->error_message);
      class_alloc(pba->w_nufld_bg[k],_QUADRATURE_MAX_BG_*sizeof(double),pba->error_message);

      class_call(get_qsampling(pba->q_nufld_bg[k],
                               pba->w_nufld_bg[k],
                               &(pba->q_size_nufld_bg[k]),
                               _QUADRATURE_MAX_BG_,
                               ppr->tol_nufld_bg,
                               pbadist.q,
                               pbadist.tablesize,
      ...
      pba->q_nufld_bg[k]=realloc(pba->q_nufld_bg[k],pba->q_size_nufld_bg[k]*sizeof(double));
      pba->w_nufld_bg[k]=realloc(pba->w_nufld_bg[k],pba->q_size_nufld_bg[k]*sizeof(double));

      /** - in verbose mode, inform user of number of sampled momenta
          for background quantities */
      if (pba->background_verbose > 0) {
        printf("nufld species i=%d sampled with %d points for purpose of background integration\n",
               k+1,
               pba->q_size_nufld_bg[k]);
      }
    }
      ...
      /** Manual q-sampling for this species. Same sampling used for both perturbation and background sampling, since this will usually be a high precision setting anyway */
      pba->q_size_nufld_bg[k] = pba->nufld_input_q_size[k];
      pba->q_size_nufld[k] = pba->nufld_input_q_size[k];
      class_alloc(pba->q_nufld_bg[k],pba->q_size_nufld_bg[k]*sizeof(double),pba->error_message);
      class_alloc(pba->w_nufld_bg[k],pba->q_size_nufld_bg[k]*sizeof(double),pba->error_message);
      class_alloc(pba->q_nufld[k],pba->q_size_nufld[k]*sizeof(double),pba->error_message);
      class_alloc(pba->w_nufld[k],pba->q_size_nufld[k]*sizeof(double),pba->error_message);
      class_call(get_qsampling_manual(pba->q_nufld[k],
                                      pba->w_nufld[k],
                                      pba->q_size_nufld[k],
                                      pba->nufld_qmax[k],
                                      pba->nufld_quadrature_strategy[k],
                                      pbadist.q,
                                      pbadist.tablesize,
                                      background_nufld_distribution,
                                      &pbadist,
                 ...
                 pba->error_message,
                 pba->error_message);
      for (index_q=0; index_q<pba->q_size_nufld[k]; index_q++) {
        pba->q_nufld_bg[k][index_q] = pba->q_nufld[k][index_q];
        pba->w_nufld_bg[k][index_q] = pba->w_nufld[k][index_q];
      }
      /** - in verbose mode, inform user of number of sampled momenta
          for background quantities */
      if (pba->background_verbose > 0) {
        printf("nufld species i=%d sampled with %d points for purpose of background andperturbation integration using the manual method\n",
               k+1,
               pba->q_size_nufld[k]);
      }
    }

    class_alloc(pba->dlnf0_dlnq_nufld[k],
                pba->q_size_nufld[k]*sizeof(double),
                pba->error_message);


    for (index_q=0; index_q<pba->q_size_nufld[k]; index_q++) {
      q = pba->q_nufld[k][index_q];
      class_call(background_nufld_distribution(&pbadist,q,&f0),
                 pba->error_message,pba->error_message);
      ...
      for (tolexp=_PSD_DERIVATIVE_EXP_MIN_; tolexp<_PSD_DERIVATIVE_EXP_MAX_; tolexp++) {
        ...
        if (index_q == 0) {
          dq = MIN((0.5-ppr->smallest_allowed_variation)*q,2*exp(tolexp)*(pba->q_nufld[k][index_q+1]-q));
        }
        else if (index_q == pba->q_size_nufld[k]-1) {
          dq = exp(tolexp)*2.0*(pba->q_nufld[k][index_q]-pba->q_nufld[k][index_q-1]);
        }
        else{
          dq = exp(tolexp)*(pba->q_nufld[k][index_q+1]-pba->q_nufld[k][index_q-1]);
        }

        class_call(background_nufld_distribution(&pbadist,q-2*dq,&f0m2),
                   pba->error_message,pba->error_message);
        class_call(background_nufld_distribution(&pbadist,q+2*dq,&f0p2),
                   pba->error_message,pba->error_message);

        if (fabs((f0p2-f0m2)/f0)>sqrt(ppr->smallest_allowed_variation)) break;
      }

      class_call(background_nufld_distribution(&pbadist,q-dq,&f0m1),
                 pba->error_message,pba->error_message);
      class_call(background_nufld_distribution(&pbadist,q+dq,&f0p1),
                 pba->error_message,pba->error_message);
      //5 point estimate of the derivative:
      df0dq = (+f0m2-8*f0m1+8*f0p1-f0p2)/12.0/dq;
      //printf("df0dq[%g] = %g. dlf=%g ?= %g. f0 =%g.\n",q,df0dq,q/f0*df0dq,
      //Avoid underflow in extreme tail:
      if (fabs(f0)==0.)
        pba->dlnf0_dlnq_nufld[k][index_q] = -q; /* valid for whatever f0 with exponential tail in exp(-q) */
      else
        pba->dlnf0_dlnq_nufld[k][index_q] = q/f0*df0dq;
    }

    pba->factor_nufld[k]=pba->deg_nufld[k]*4*_PI_*pow(pba->T_cmb*pba->T_nufld[k]*_k_B_,4)*8*_PI_*_G_
      /3./pow(_h_P_/2./_PI_,3)/pow(_c_,7)*_Mpc_over_m_*_Mpc_over_m_;
 ...
 * omega_nufld in input but not the mass, infer the mass with Newton iteration method.
 *
 * @param ppr    Input: precision structure
 * @param pba    Input/Output: background structure
 * @param n_nufld Input: index of nufld species
 */
...
int background_checks(
  ...
  if (pba->background_verbose > 0) {
    ...
    if (pba->has_nufld == _TRUE_) {
      ...
      /* loop over nufld species */
      for (n_nufld=0;n_nufld<pba->N_nufld; n_nufld++) {

        /* inform if p-s-d read in files */
        if (pba->got_files[n_nufld] == _TRUE_) {
          printf(" -> nufld species i=%d read from file %s\n",n_nufld+1,pba->nufld_psd_files+filenum*_ARGUMENT_LENGTH_MAX_);
          filenum++;
        }
        ...
        /* inform user of the contribution of each species to
           radiation density (in relativistic limit): should be
           between 1.01 and 1.02 for each active neutrino species;
           evaluated as rho_nufld/rho_nu_rel where rho_nu_rel is the
           density of one neutrino in the instantaneous decoupling
           limit, i.e. assuming T_nu=(4/11)^1/3 T_gamma (this comes
           from the definition of N_eff) */
        rho_nu_rel = 56.0/45.0*pow(_PI_,6)*pow(4.0/11.0,4.0/3.0)*_G_/pow(_h_P_,3)/pow(_c_,7)*
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_nufld == _TRUE_) {
    ...
    class_test(counter == _MAX_IT_,
               pba->error_message,
               "Search for initial scale factor a such that all nufld species are relativistic failed.");
  }
  ...
  if (pba->has_nufld == _TRUE_) {
    /** - We must add the relativistic contribution from nufld species */
    rho_rad += rho_nufld_rel_tot;
  }
  if (pba->has_dcdm == _TRUE_) {
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if (pba->N_nufld > 0) {
      for (index_nufld=0;index_nufld<pba->N_nufld;++index_nufld) {
        printf("-> %-26s%-4d Omega = %-15g , omega = %-15g\n","Non-Cold Species Nr.",index_nufld+1,pba->Omega0_nufld[index_nufld],pba->Omega0_nufld[index_nufld]*pba->h*pba->h);
        budget_neutrino+=pba->Omega0_nufld[index_nufld];
        budget_matter+=pba->Omega0_nufld[index_nufld];
      }
    }
