...
struct background
{
  ...
  int N_nufld;                            /**< Number of distinguishable nufld species */
  /* the following parameters help to define tabulated nufld p-s-d passed in file */
  char * nufld_psd_files;                 /**< list of filenames for tabulated p-s-d */
  // int * got_files;                       /**< nufld_MOD: What about this? list of flags for each species, set to true if p-s-d is passed through file */
  /* the following parameters help to define the analytical nufld phase space distributions (p-s-d) */
  double * nufld_psd_parameters;          /**< list of parameters for specifying/modifying nufld p.s.d.'s, to be customized for given model
                                            (could be e.g. mixing angles) */
  double * M_nufld;                       /**< vector of masses of non-cold relic: dimensionless ratios m_nufld/T_nufld */
  double * m_nufld_in_eV;                 /**< list of nufld masses in eV (inferred from M_nufld and other parameters above) */
  double * Omega0_nufld, Omega0_nufld_tot; /**< Omega0_nufld for each species and for the total Omega0_nufld */
  double * T_nufld,T_nufld_default;        /**< list of 1st parameters in p-s-d of non-cold relics: relative temperature
                                            T_nufld1/T_gamma; and its default value */
  double * ksi_nufld, ksi_nufld_default;   /**< list of 2nd parameters in p-s-d of non-cold relics: relative chemical potential
                                            ksi_nufld1/T_nufld1; and its default value */
  double * deg_nufld, deg_nufld_default;    /**< vector of degeneracy parameters in factor of p-s-d: 1 for one family of neutrinos
                                             (= one neutrino plus its anti-neutrino, total g*=1+1=2, so deg = 0.5 g*); and its
                                             default value */
  ...
  int index_bg_w_nufld_mass1;  /**< equation of state of first nufld species, from standard mass (others contiguous)*/
  int index_bg_w_prime_nufld_mass1;  /**< derivative wrt conformal time of equation of state of first nufld species, from standard mass (others contiguous)*/
  ...
  short has_nufld;      /**< presence of non-cold dark matter with general boltzmann hierarchy? */
  ...
  /**
   *@name - arrays related to sampling and integration of nufld phase space distributions
   */

  //@{

  int * nufld_quadrature_strategy; /**< Vector of integers according to quadrature strategy. */
  double ** q_nufld_bg;  /**< Pointers to vectors of background sampling in q */
  double ** w_nufld_bg;  /**< Pointers to vectors of corresponding quadrature weights w */
  double ** q_nufld;     /**< Pointers to vectors of perturbation sampling in q */
  double ** w_nufld;     /**< Pointers to vectors of corresponding quadrature weights w */
  double ** dlnf0_dlnq_nufld; /**< Pointers to vectors of logarithmic derivatives of p-s-d */
  int * q_size_nufld_bg; /**< Size of the q_nufld_bg arrays */
  int * q_size_nufld;    /**< Size of the q_nufld arrays */
  double * factor_nufld; /**< List of normalization factors for calculating energy density etc.*/
...
// nufld_MOD: These lines have not been changed (yet), we can use the same lines as in ncdm
